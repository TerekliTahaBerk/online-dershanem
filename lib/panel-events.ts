...
export const panelEventSchema = z.discriminatedUnion("name", [
  ...
  z.object({ name: z.literal("odk_attempt_started"), properties: z.object({ family: z.string().min(2).max(80), resumed: z.boolean(), lateEntryBand: z.enum(["ON_TIME", "1-5M", "6M+"]) }).strict() }),
  z.object({ name: z.literal("odk_attempt_submitted"), properties: z.object({ family: z.string().min(2).max(80), mode: z.enum(["MANUAL", "AUTO"]), answeredBand: z.enum(["0", "1-10", "11-20", "21-40"]), durationBand: z.enum(["0-30M", "31-60M", "61M+"]) }).strict() }),
  z.object({ name: z.literal("odk_exam_scored"), properties: z.object({ family: z.string().min(2).max(80), attemptBand: z.enum(["0", "1-10", "11-50", "51+"]) }).strict() }),
  z.object({ name: z.literal("odk_results_released"), properties: z.object({ family: z.string().min(2).max(80), attemptBand: z.enum(["0", "1-10", "11-50", "51+"]) }).strict() }),
  z.object({
