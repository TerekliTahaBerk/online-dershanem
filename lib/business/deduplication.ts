...
export function leadMatchConfidence(left: LeadIdentity, right: LeadIdentity) {
  ...
  if (user || instagram || (phone && email)) return { confidence: 1, autoMerge: true, reasons: [user ? "USER" : null, instagram ? "INSTAGRAM" : null, phone ? "PHONE" : null, email ? "EMAIL" : null].filter(Boolean) };
  if (phone || email) return { confidence: 0.78, autoMerge: false, reasons: [phone ? "PHONE" : "EMAIL"] };
  return { confidence: 0, autoMerge: false, reasons: [] };
}
