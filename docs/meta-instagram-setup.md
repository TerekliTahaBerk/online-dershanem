...
5. App secret, user token ve account id’yi Vercel server env’e ekleyin. Marketing API için ayrı olarak `META_ADS_ACCESS_TOKEN` ve `META_AD_ACCOUNT_ID` kullanılır.
...
Test event’inden sonra sırasıyla `InstagramWebhookEvent`, `BackgroundJob`, `BusinessConversation`, `BusinessMessage` kontrol edilir. Mesaj delivery/read callback’leri ilgili `BusinessMessage.status` ve `MessageDelivery` geçmişini günceller. `/api/integrations/instagram/health` token ile `me?id,username` çağrısını yapar; 401 imza/app secret, 403 verify token, `META_SEND_*` token/izin/sürüm sorununa işaret eder. Token yenileme sonucunun yeni secret değeri Vercel’de operatör tarafından kaydedilip yeniden deploy edilmelidir; uygulama runtime secret’ını kendisi değiştirmez.
