...
export function SectionIntro({
  ...
  return (
    <header className={cn("public-intro", `public-intro-${align}`, className)}>
      ...
      <h2
        id={titleId}
        className={cn("public-heading", `public-heading-${size}`)}
      >
        {title}
      </h2>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        `public-button-${size}`,
        mobileFull && "public-button-mobile-full",
        className,
      )}
    >
      {children}
...
export function PublicBadge({
  ...
  children: ReactNode;
  tone?: "olive" | "neutral" | "yellow" | "sky";
  className?: string;
}) {
  return (
    <span className={cn("public-badge", `public-badge-${tone}`, className)}>
      {children}
    </span>
  );
}
...
export function PublicCard({
  ...
  return (
    <Component
      className={cn(
        "public-card",
        interactive && "public-card-interactive",
        className,
      )}
    >
      {children}
...
export function ComparisonRows({
  ...
  return (
    <div className="public-comparison">
      <div
        className="public-comparison-desktop"
        role="region"
        tabIndex={0}
        aria-label={`${caption}; yatay kaydırılabilir`}
      >
        <table>
            ...
            <tr>
              ...
              {columns.map((column, index) => (
                <th
                  ...
                  scope="col"
                  className={
                    index === highlightColumn ? "is-highlighted" : undefined
                  }
                >
                  {column}
                </th>
      ...
      <div className="public-comparison-mobile" aria-label={caption}>
        {rows.map((row) => (
          <section
            key={row.label}
            className="public-comparison-card"
            aria-label={`${caption}: ${row.label}`}
          >
            <h3>{row.label}</h3>
            <dl>
              {columns.map((column, index) => (
...
export function PricingValueBlock({
  before,
  value,
  suffix,
  note,
  ...
  return (
    <div className={cn("public-value-block", className)}>
      ...
      <div className="public-value-line">
        <strong>{value}</strong>
  ...
  return (
    <div
      className={cn(
        ...
        reverse && "public-feature-story-reverse",
      )}
    >
    ...
    <PublicSection tone="white" space="default">
      <div className="public-final-cta">
        ...
        <div className="public-final-actions">
          <PublicButton href={href} size="lg" mobileFull>
            {label}
