...
type Row = {
  ...
  package: null | {
    ...
    dueAt: string;
    items: { id: string; kind: "MATERIAL" | "ASSIGNMENT"; title: string }[];
  };
};

export function TeacherRecoveryManager({ rows }: { rows: Row[] }) {
  const [busy, setBusy] = useState<string | null>(null);
  const [message, setMessage] = useState("");
  async function generate(row: Row) {
    setBusy(row.attendanceId);
    setMessage("");
    const response = await fetch("/api/panel/recovery-packages/generate", {
      method: "POST",
    ...
    if (!response.ok) {
      setMessage(body.error || "Paket hazırlanamadı.");
      setBusy(null);
      return;
    }
  ...
  async function publish(row: Row) {
    ...
    setBusy(row.package.id);
    setMessage("");
    const response = await fetch(
      `/api/panel/recovery-packages/${row.package.id}/publish`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ expectedVersion: row.package.version }),
      },
    );
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setMessage(body.error || "Paket yayınlanamadı.");
      setBusy(null);
      return;
    }
    setMessage(
      body.planRebalanced
        ? "Paket yayınlandı; onaylı haftalık plan kapasite sınırlarıyla yeniden dengelendi."
        : "Paket öğrenciye yayınlandı.",
    );
    window.location.reload();
  }
  return (
          ...
          <article key={row.attendanceId} className="panel-surface p-5 sm:p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                <RefreshCcw size={14} />{" "}
                {row.package
                  ? "Taslağı güvenle yenile"
                  : "Telafi taslağı hazırla"}
              </button>
              {row.package?.status === "DRAFT" ? (
                <button
                  type="button"
