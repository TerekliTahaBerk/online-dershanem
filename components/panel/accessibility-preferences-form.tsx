...
const choices = [
  {
    ...
    body: "Animasyon ve geçişleri en aza indirir.",
    icon: Move,
  },
  {
    key: "highContrast",
    title: "Yüksek kontrast",
    body: "Metin ve sınırları daha belirgin yapar.",
    icon: Eye,
  },
  {
    key: "largeText",
    title: "Büyük metin",
    body: "Panel metinlerini yaklaşık %12,5 büyütür.",
    icon: Text,
  },
  {
    key: "comfortableSpacing",
    title: "Rahat satır aralığı",
    body: "Metin bloklarında okuma aralığını artırır.",
    icon: AlignJustify,
  },
  {
    key: "captionsPreferred",
    title: "Altyazılı medya",
    body: "Öğretmenin uygun medya hazırlarken bu ihtiyacı görür.",
    icon: Captions,
  },
  {
    key: "transcriptPreferred",
    title: "Metin dökümü",
    body: "Video ve ses içeriğinde metin alternatifini tercih eder.",
    icon: FileText,
  },
] as const;
...
export function AccessibilityPreferencesForm({
  ...
  function enabled(key: (typeof choices)[number]["key"]) {
    return key === "largeText"
      ? preference.textScale === "LARGE"
      : preference[key];
  }
  function toggle(key: (typeof choices)[number]["key"]) {
    setPreference((current) =>
      key === "largeText"
        ? {
            ...current,
            textScale: current.textScale === "LARGE" ? "DEFAULT" : "LARGE",
          }
  ...
  async function save() {
    setBusy(true);
    setMessage("");
    const response = await fetch("/api/panel/accessibility/preferences", {
      method: "PATCH",
    ...
    setBusy(false);
    if (!response.ok)
      return setMessage(data.error || "Tercihler kaydedilemedi.");
    applyAccessibilityViewPreference(preference);
    setPreference((current) => ({ ...current, version: data.version }));
    setMessage("Tercihler kaydedildi ve panele uygulandı.");
    router.refresh();
  }
  return (
    <section className="panel-surface p-5">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {choices.map((choice) => {
          ...
          const active = enabled(choice.key);
          return (
            <button
              type="button"
              key={choice.key}
