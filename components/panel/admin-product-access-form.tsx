...
export function AdminProductAccessForm({
  userId,
  role,
  initialProducts,
}: {
  ...
  async function save() {
    ...
    setBusy(true);
    setMessage("");
    const response = await fetch(`/api/panel/users/${userId}/products`, {
      method: "PUT",
    ...
    setBusy(false);
    if (!response.ok)
      return setMessage(body.error || "Ürün erişimi güncellenemedi.");
    setMessage("Ürün erişimi güncellendi.");
    router.refresh();
  }
