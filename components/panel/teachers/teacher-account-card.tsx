...
/**
 * Phase 3 / Session 4 — D4: Teacher account lifecycle card (client).
 *
 * Operational widget for the teacher detail page. Wraps the unified
 * `User`-flow account actions (Session 1–2 helpers) bound to a teacher.
 * Mirrors `parent-account-card.tsx`.
 */
...
import {
  ...
  regenerateTeacherUserInviteAction,
  revokeTeacherUserInviteAction,
  disableTeacherAccountAction,
  enableTeacherAccountAction,
  forceTeacherPasswordChangeAction,
  resetTeacherTempPasswordAction,
...
export function TeacherAccountCard({ teacherId, email, hasAccount, accountState, user }: Props) {
  ...
  const run = (label: string, fn: () => Promise<unknown>) => {
    ...
    startTransition(async () => {
      try {
        const res = (await fn()) as { url?: string; tempPassword?: string } | undefined;
        if (res?.url) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div className="od-grid g-2" style={{ gap: 8, fontSize: 12, color: "var(--od-muted)" }}>
        ...
        <Kv k="Şifre değiştirme zorunlu" v={user?.mustChangePassword ? "Evet" : "Hayır"} />
        ...
        {!hasAccount ? (
          <>
            <button
              ...
              onClick={() => {
                ...
                run("Davet üret", () => createTeacherAccountAction(teacherId, fd));
              }}
            >
              Hesap aç + davet üret
            </button>
            <button
              ...
              onClick={() => {
                ...
                run("Geçici şifre", () => createTeacherAccountAction(teacherId, fd));
              }}
            >
            ...
            {!email ? (
              <span style={{ fontSize: 12, color: "var(--pd-warn, #b45309)" }}>
                Önce öğretmenin email&apos;ini doldurun.
              </span>
            ) : null}
          </>
        ) : (
          <>
            <button
              ...
              className="od-btn ghost sm"
              disabled={pending}
              onClick={() => run("Davet yenilendi", () => regenerateTeacherUserInviteAction(teacherId))}
            >
            ...
            {accountState === "INVITE_PENDING" || accountState === "INVITE_EXPIRED" ? (
              <button
                ...
                className="od-btn ghost sm"
                disabled={pending}
                onClick={() => run("Davet iptal", () => revokeTeacherUserInviteAction(teacherId))}
              >
                Daveti iptal et
              </button>
            ) : null}
            <button
              ...
              className="od-btn ghost sm"
              disabled={pending}
              onClick={() => run("Geçici şifre", () => resetTeacherTempPasswordAction(teacherId))}
            >
              Geçici şifre ver
            </button>
            <button
              ...
              className="od-btn ghost sm"
              disabled={pending}
              onClick={() => run("Şifre değişimi zorunlu", () => forceTeacherPasswordChangeAction(teacherId))}
            >
            ...
            {accountState === "DISABLED" ? (
              <button
                ...
                className="od-btn ghost sm"
                disabled={pending}
                onClick={() => run("Hesap aktifleştir", () => enableTeacherAccountAction(teacherId))}
              >
                Hesabı aktifleştir
              </button>
            ) : (
              <button
                ...
                disabled={pending}
                onClick={() => run("Hesap devre dışı", () => disableTeacherAccountAction(teacherId))}
              >
