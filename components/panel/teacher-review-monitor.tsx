...
export function TeacherReviewMonitor({ students }: { students: StudentRow[] }) {
  ...
  const totalDue = students.reduce((sum, student) => sum + student.dueCount, 0);
  const totalActive = students.reduce(
    (sum, student) => sum + student.activeCount,
    0,
  );
  useEffect(() => {
    sendPanelEvent({
      ...
      properties: {
        actorRole: "TEACHER",
        dueCountBand: band(totalDue),
        activeCountBand: band(totalActive),
      },
    });
  }, [totalActive, totalDue]);
  async function create() {
    const response = await fetch("/api/panel/review-queue", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ studentId, title, sourceReference }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok)
      return setMessage(body.error || "Tekrar öğesi oluşturulamadı.");
    setMessage("Kaynak referansı yarının küçük tekrarlarına eklendi.");
    setTitle("");
    setSourceReference("");
  }
  return (
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <label className="panel-field">
            ...
            <input
              ...
              placeholder="Örn. Öğretmen föyü s.4, soru 6"
            />
          </label>
        </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {needsAttention ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-[9px] font-extrabold text-amber-900">
                        <AlertTriangle size={11} /> İnsan bakışı gerekli
                      </span>
                    ) : null}
                  </div>
                  <p className="mt-1 text-xs text-[var(--site-muted)]">
