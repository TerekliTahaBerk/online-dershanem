...
export function NetworkPreferencesForm({ initial }: { initial: Preference }) {
  ...
  async function save() {
    setBusy(true);
    setMessage("");
    const response = await fetch("/api/panel/network/preferences", {
      method: "PATCH",
    ...
    setBusy(false);
    if (!response.ok)
      return setMessage(body.error || "Tercihler kaydedilemedi.");
    document.documentElement.dataset.panelLowData = preference.lowDataMode
      ? "true"
      : "false";
    offline.applyPreferences(
      preference.lowDataMode,
      preference.offlineWritesEnabled,
    );
    ...
    <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
      <section className="panel-surface p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-3">
          <button
            type="button"
            aria-pressed={preference.lowDataMode}
            onClick={() =>
          ...
          >
            ...
            <span className="ml-auto text-xs font-bold">
              {preference.lowDataMode ? "Açık" : "Kapalı"}
            </span>
          </button>
          <button
            type="button"
            aria-pressed={preference.offlineWritesEnabled}
            onClick={() =>
              setPreference((current) => ({
                ...current,
          ...
          >
            ...
            <span className="ml-auto text-xs font-bold">
              {preference.offlineWritesEnabled ? "Açık" : "Kapalı"}
            </span>
          </button>
        </div>
        <p
