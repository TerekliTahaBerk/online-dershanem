          ...
          {groups.map((group) => (
            ...
            >
              <summary className="flex cursor-pointer list-none items-center gap-3 p-4">
                <span
                  className={`grid h-9 w-9 place-items-center rounded-xl ${group.isActive ? "bg-[#e5efe2] text-[#365630]" : "bg-slate-100 text-slate-500"}`}
                >
                  <UsersRound size={17} />
                </span>
          ...
          {lessons.map((lesson) => (
            ...
            >
              <summary className="flex cursor-pointer list-none items-center gap-3 p-4">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-[9px] font-extrabold ${lesson.status === "CANCELLED" ? "bg-rose-50 text-rose-700" : lesson.status === "COMPLETED" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}
                >
              ...
              <form
                ...
                onSubmit={(event) => {
                  ...
                  if (
                    willCancel &&
                    !window.confirm("Bu ders iptal edilsin mi?")
                  )
                    return;
                  void run(
                    lesson.id,
                    async () => {
                      ...
                      if (action === "MAKE_UP") {
                        await patch(`/api/panel/lessons/${lesson.id}`, {
                          ...
                          teacherId:
                            data.get("substituteTeacherId") || undefined,
