...
type SubmitResult = {
  state: "synced" | "queued" | "conflict" | "error";
...
function attemptBand(attempts: number): "1" | "2-3" | "4+" {
...
export function OfflineSyncProvider({
    ...
    try {
      ...
      for (const record of records.filter((item) => item.state === "QUEUED")) {
        if (record.expiresAt <= Date.now()) {
          ...
          sendPanelEvent({
            name: "offline_write_queued",
            properties: {
              operation: record.kind,
              payloadSizeBand: payloadSizeBand(record.body),
            },
          });
          sendPanelEvent({
            name: "offline_write_conflicted",
            properties: { operation: record.kind, conflictType: "EXPIRED" },
          });
          window.dispatchEvent(
            new CustomEvent("panel-offline-conflict", {
              detail: { kind: record.kind, url: record.url },
            }),
          );
          continue;
        }
        ...
        if (response?.ok) {
          ...
          sendPanelEvent({
            name: "offline_write_synced",
            properties: {
              operation: record.kind,
              queueAgeBand: queueAgeBand(record.createdAt),
              attemptBand: attemptBand(record.attempts + 1),
            },
          });
          window.dispatchEvent(
            new CustomEvent("panel-offline-synced", {
              detail: {
                kind: record.kind,
                url: record.url,
                body,
        ...
        if (response?.status === 409) {
          ...
          sendPanelEvent({
            name: "offline_write_conflicted",
            properties: { operation: record.kind, conflictType: "VERSION" },
          });
          window.dispatchEvent(
            new CustomEvent("panel-offline-conflict", {
              detail: { kind: record.kind, url: record.url },
        ...
        if (response && [400, 401, 403, 404].includes(response.status)) {
          ...
          sendPanelEvent({
            name: "offline_write_conflicted",
            properties: { operation: record.kind, conflictType: "REJECTED" },
          });
          window.dispatchEvent(
            new CustomEvent("panel-offline-conflict", {
              detail: { kind: record.kind, url: record.url },
        ...
        await putOfflineMutation({
          ...record,
          attempts: Math.min(10, record.attempts + 1),
        });
        break;
      }
    } finally {
  ...
  useEffect(() => {
    ...
    void refreshCounts();
    if (available && "serviceWorker" in navigator)
      void navigator.serviceWorker
        .register("/panel-sw.js", { scope: "/" })
        .catch(() => undefined);
    return () => {
  ...
  useEffect(() => {
    document.documentElement.dataset.panelLowData = dataSaver
      ? "true"
      : "false";
    return () => {
      document.documentElement.removeAttribute("data-panel-low-data");
    };
  }, [dataSaver]);

  const submitMutation = useCallback(
    async (input: OfflineMutationInput): Promise<SubmitResult> => {
      let response: Response | null = null;
      if (navigator.onLine) {
        try {
      ...
      if (!writesEnabled)
        return {
          ...
          response,
          body: {
            error: "Bağlantı kurulamadı; çevrimdışı kayıt bu hesapta kapalı.",
          },
        };
      try {
        ...
        return {
          state: "error",
          response: null,
          body: { error: "Bu işlem güvenli çevrimdışı kuyruğa alınamadı." },
        };
      }
    },
    [refreshCounts, scope, writesEnabled],
  ...
  const clearDeviceQueue = useCallback(async () => {
    if (scope) await clearOfflineScope(scope);
    await refreshCounts();
  }, [refreshCounts, scope]);
  const applyPreferences = useCallback(
    (nextLowDataMode: boolean, nextOfflineWritesEnabled: boolean) => {
      setDataSaver(nextLowDataMode);
      setWritesEnabled(nextOfflineWritesEnabled);
    },
    [],
  );
  const value = useMemo(
    ...
    <OfflineContext.Provider value={value}>
      {!online || queuedCount || conflictCount ? (
        ...
        >
          {!online ? (
            <>
              <CloudOff size={14} /> Çevrimdışısınız.
            </>
          ) : (
            <>
              <RefreshCw size={14} /> Bağlantı geri geldi.
            </>
          )}
          {queuedCount ? (
            <span>{queuedCount} güvenli işlem cihazda bekliyor.</span>
          ) : null}
          {conflictCount ? (
            <>
              <TriangleAlert size={14} />
              <span>{conflictCount} işlem insan kontrolü istiyor.</span>
            </>
          ) : null}
          <Link
            href="/panel/veri-kullanimi"
