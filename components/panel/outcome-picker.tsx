...
export type SelectedOutcome = {
  outcomeId: string;
  evidenceType: "TAUGHT" | "OBSERVED" | "INDEPENDENT" | "NEEDS_REVIEW";
};

const evidenceLabels = {
  TAUGHT: "İşlendi",
  OBSERVED: "Gözlendi",
  INDEPENDENT: "Bağımsız uyguladı",
...
export function OutcomePicker({
  ...
  onChange: (value: SelectedOutcome[]) => void;
  withEvidence?: boolean;
}) {
  const [query, setQuery] = useState("");
  const [favorites, setFavorites] = useState(
    () =>
      new Set(outcomes.filter((item) => item.favorite).map((item) => item.id)),
  );
  const [remoteOutcomes, setRemoteOutcomes] = useState(outcomes);
  const [loading, setLoading] = useState(false);
  const normalized = query.trim().toLocaleLowerCase("tr-TR");
  useEffect(() => {
    setRemoteOutcomes(outcomes);
    setFavorites(
      new Set(outcomes.filter((item) => item.favorite).map((item) => item.id)),
    );
  }, [outcomes]);
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(
        `/api/panel/curriculum/outcomes/search?query=${encodeURIComponent(query)}&limit=8`,
        ...
        .then((body: { outcomes: OutcomeOption[] }) =>
          setRemoteOutcomes(body.outcomes),
        )
        .catch(() => undefined)
  ...
  const source = normalized ? remoteOutcomes : outcomes;
  const results = useMemo(
    () =>
      source
        .filter(
          (item) =>
            !value.some((selected) => selected.outcomeId === item.id) &&
            (!normalized ||
              `${item.code} ${item.title} ${item.subject} ${item.unit} ${item.skills.join(" ")}`
        ...
        .slice(0, 8),
    [favorites, normalized, source, value],
  );
  ...
  async function toggleFavorite(outcomeId: string) {
    ...
    setFavorites((current) => {
      ...
      if (favorite) next.add(outcomeId);
      else next.delete(outcomeId);
      return next;
    });
    await fetch("/api/panel/teacher/outcome-favorites", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ outcomeId, favorite }),
    }).catch(() => undefined);
  }
    ...
    <div className="rounded-2xl border border-[var(--site-line)] bg-[var(--site-bg-warm)] p-4">
      ...
      <div className="mt-3 space-y-2">
        {value.map((selected) => {
          const outcome = outcomes.find(
            (item) => item.id === selected.outcomeId,
          );
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <button
                  type="button"
                  aria-label={`${outcome.code} kazanımını kaldır`}
                  onClick={() =>
                    onChange(
                      value.filter(
                        (item) => item.outcomeId !== selected.outcomeId,
                      ),
                    )
                  }
              ...
              {withEvidence ? (
                <select
                  aria-label={`${outcome.code} kanıt türü`}
                  value={selected.evidenceType}
                  onChange={(event) =>
                    onChange(
                      value.map((item) =>
                        item.outcomeId === selected.outcomeId
                          ? {
                              ...item,
                              evidenceType: event.target
                                .value as SelectedOutcome["evidenceType"],
                            }
                          : item,
                      ),
                    )
      ...
      {value.length < 3 ? (
        <>
          ...
          {query || outcomes.length <= 8 ? (
            <div className="mt-2 max-h-64 space-y-1 overflow-y-auto">
              {results.map((outcome) => (
                <div
                  key={outcome.id}
                  className="flex items-start gap-2 rounded-xl bg-white p-2.5"
                >
                  <button
                    type="button"
                    onClick={() => {
                      onChange([
                        ...value,
                        { outcomeId: outcome.id, evidenceType: "TAUGHT" },
                      ]);
                      setQuery("");
                    }}
                  ...
                  <button
                    type="button"
                    aria-label={`${outcome.code} favori`}
                    aria-pressed={favorites.has(outcome.id)}
                    onClick={() => void toggleFavorite(outcome.id)}
                    className={
                      favorites.has(outcome.id)
                        ? "text-amber-600"
                        : "text-slate-300"
                    }
                  >
                    <Star
                      size={15}
                      fill={favorites.has(outcome.id) ? "currentColor" : "none"}
                    />
                  </button>
                </div>
              ))}
