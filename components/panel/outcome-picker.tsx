...
export function OutcomePicker({ outcomes, value, onChange, withEvidence = false }: { outcomes: OutcomeOption[]; value: SelectedOutcome[]; onChange: (value: SelectedOutcome[]) => void; withEvidence?: boolean }) {
  ...
  useEffect(() => { setRemoteOutcomes(outcomes); setFavorites(new Set(outcomes.filter((item) => item.favorite).map((item) => item.id))); }, [outcomes]);
  useEffect(() => {
    if (!normalized) return;
    const controller = new AbortController();
    setLoading(true);
    const timer = window.setTimeout(() => {
      void fetch(`/api/panel/curriculum/outcomes/search?query=${encodeURIComponent(query)}&limit=8`, { signal: controller.signal })
        .then((response) => response.ok ? response.json() : Promise.reject(new Error("search_failed")))
        .then((body: { outcomes: OutcomeOption[] }) => setRemoteOutcomes(body.outcomes))
        .catch(() => undefined)
  ...
  const results = useMemo(() => source.filter((item) => !value.some((selected) => selected.outcomeId === item.id) && (!normalized || `${item.code} ${item.title} ${item.subject} ${item.unit} ${item.skills.join(" ")}`.toLocaleLowerCase("tr-TR").includes(normalized))).sort((a, b) => Number(favorites.has(b.id)) - Number(favorites.has(a.id)) || Number(b.recent) - Number(a.recent)).slice(0, 8), [favorites, normalized, source, value]);
