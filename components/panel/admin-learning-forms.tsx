...
export function AdminLearningForms({ teachers, students, parents, groups }: { teachers: Person[]; students: Student[]; parents: Person[]; groups: Group[] }) {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <form id="ders-planla" className="panel-action-card scroll-mt-28" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const local = String(data.get("startsAt")); void submit(() => post("/api/panel/lessons", { groupId: data.get("groupId"), title: data.get("title"), startsAt: new Date(local).toISOString() }), "Ders takvime eklendi."); }}>
