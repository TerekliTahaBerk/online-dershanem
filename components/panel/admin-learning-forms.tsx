...
export function AdminLearningForms({ teachers, students, parents, groups }: { teachers: Person[]; students: Student[]; parents: Person[]; groups: Group[] }) {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-3">
      <form className="panel-action-card" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const selected = data.getAll("studentIds").map(String); void submit(() => post("/api/panel/groups", { name: data.get("name"), subject: data.get("subject"), level: data.get("level"), teacherId: data.get("teacherId"), studentIds: selected }), "Grup hazır."); }}>
      ...
      <form className="panel-action-card" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const local = String(data.get("startsAt")); void submit(() => post("/api/panel/lessons", { groupId: data.get("groupId"), title: data.get("title"), startsAt: new Date(local).toISOString() }), "Ders takvime eklendi."); }}>
