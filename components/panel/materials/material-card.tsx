...
export function MaterialCard({ material: m, editHref, hideContext }: Props) {
  ...
  return (
    <div
      className={`od-material-card${m.isArchived ? " is-archived" : ""}${!m.isPublished ? " is-draft" : ""}`}
    >
      <div className="od-row od-row-between" style={{ alignItems: "flex-start", gap: 12 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {!hideContext ? (
            <div className="od-content-meta" style={{ marginTop: 8 }}>
        ...
        <div className="od-content-action-row" style={{ flexShrink: 0 }}>
          {openUrl ? (
            <a
          ...
          {editHref ? (
            <Link className="od-btn ghost sm" href={editHref}>
              Düzenle
            </Link>
          ) : null}
        </div>
