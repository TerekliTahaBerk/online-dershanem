...
export function AdminSetupWizard({
  ...
  const [parentByStudent, setParentByStudent] = useState<
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-[var(--site-line)] bg-[var(--site-soft)] p-5 sm:p-6">
        ...
        <form
          className="grid gap-2 md:grid-cols-[150px_1fr_1fr_auto]"
          onSubmit={async (event) => {
            ...
            const role = String(data.get("role")) as SetupRole;
            ...
            if (!response.ok)
              return setAccountMessage(result.error || "Hesap açılamadı.");
            const person = {
              id:
                role === "STUDENT"
                  ? result.user.studentProfileId
                  : result.user.id,
              name: result.user.fullName || result.user.email,
            };
            if (!person.id)
              return setAccountMessage("Öğrenci profili oluşturulamadı.");
            setPeople((current) =>
              role === "TEACHER"
                ? { ...current, teachers: [...current.teachers, person] }
                : role === "STUDENT"
                  ? { ...current, students: [...current.students, person] }
                  : { ...current, parents: [...current.parents, person] },
            );
            if (role === "STUDENT")
              setSelected((current) =>
                current.length < DEFAULT_GROUP_CAPACITY
                  ? [...current, person.id]
                  : current,
              );
            if (!result.invite?.url || !result.invite?.expiresAt)
              return setAccountMessage("Davet bağlantısı üretilemedi.");
            ...
            setAccountMessage(
              `${person.name} hesabı hazır ve seçimlere eklendi.`,
            );
            form.reset();
          }}
        >
        ...
        {invitations.length ? (
          <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
            <p className="text-[10.5px] font-extrabold uppercase tracking-wide text-amber-800">
              Davet bağlantılarını şimdi iletin · süre sonunda yenilemeniz
              gerekir
            </p>
      ...
      <form
        ...
        onSubmit={async (event) => {
          ...
          if (
            ...
            return setMessage("Grup adı, ders ve öğretmen zorunlu.");
          if (step === 2 && !selected.length)
            return setMessage("En az bir öğrenci seçin.");
          if (step < 3) return setStep(step + 1);
          if (!data.get("lessonTitle") || !data.get("startsAt"))
            return setMessage("Ders başlığı ve ilk ders zamanı zorunlu.");
          setBusy(true);
          const response = await fetch("/api/panel/setup", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              name: data.get("name"),
              ...
              studentIds: selected,
              parentLinks: selected
                .filter((id) => parentByStudent[id])
                .map((studentId) => ({
                  studentId,
                  parentId: parentByStudent[studentId],
                })),
              lessonTitle: data.get("lessonTitle"),
              startsAt: new Date(String(data.get("startsAt"))).toISOString(),
              repeatWeeks: Number(data.get("repeatWeeks")),
          ...
          setBusy(false);
          if (!response.ok)
            return setMessage(result.error || "Kurulum tamamlanamadı.");
          setMessage(`${result.lessonCount} haftalık dersle grup hazır.`);
          router.refresh();
        }}
      >
          ...
          {people.students.map((student) => {
            ...
            return (
              ...
              >
                ...
                {active ? (
                  <select
                    aria-label={`${student.name} velisi`}
                    value={parentByStudent[student.id] || ""}
                    onChange={(event) =>
                      setParentByStudent({
                        ...parentByStudent,
