...
type Credential = { name: string; email: string; password: string; role: SetupRole };
  ...
  return <section id="hizli-kurulum" className="panel-surface scroll-mt-24 overflow-hidden">
    ...
    <div className="border-b border-[var(--site-line)] bg-[var(--site-soft)] p-5 sm:p-6">
      ...
      <form className="grid gap-2 md:grid-cols-[150px_1fr_1fr_auto]" onSubmit={async (event) => {
        ...
        const response = await fetch("/api/panel/users", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ role, fullName: data.get("fullName"), email: data.get("email") }) });
        ...
        if (role === "STUDENT") setSelected((current) => current.length < 4 ? [...current, person.id] : current);
        setCredentials((current) => [...current, { name: person.name, email: result.user.email, password: result.tempPassword, role }]);
      ...
      {credentials.length ? <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3"><p className="text-[10.5px] font-extrabold uppercase tracking-wide text-amber-800">Geçici parolaları şimdi güvenli biçimde kaydedin · tekrar gösterilemez</p><div className="mt-2 grid gap-2 md:grid-cols-2">{credentials.map((item) => <div key={item.email} className="rounded-xl bg-white px-3 py-2 text-xs text-[var(--site-body)]"><strong>{item.name}</strong><br />{item.email} · <code className="font-extrabold text-[var(--site-ink)]">{item.password}</code></div>)}</div></div> : null}
    </div>
