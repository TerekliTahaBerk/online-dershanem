...
import type { ParentUpcomingLesson } from "@/lib/panel/parent-dashboard";
...
type Props = { lessons: ParentUpcomingLesson[] };

export function ParentUpcomingLessons({ lessons }: Props) {
  const byDay = new Map<string, ParentUpcomingLesson[]>();
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle={lessons.length > 0 ? `${lessons.length} ders · 7 gün` : undefined}
        right={
      ...
      <CardBody>
        {lessons.length === 0 ? (
          <EmptyState icon="cal" title="Önümüzdeki 7 günde ders yok." />
        ) : (
                        ...
                        <div key={l.id} style={{
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <div className="od-muted" style={{ fontSize: 11, display: "flex", gap: 6, flexWrap: "wrap", marginTop: 2 }}>
                              ...
                              {l.classroomName ? <span>· {l.classroomName}</span> : null}
                              {l.teacherName ? <span>· {l.teacherName}</span> : null}
                            </div>
                          </div>
