...
export function StudentRecoveryPackages({ rows }: { rows: Row[] }) {
  ...
  async function completeItem(packageId: string, itemId: string) {
    ...
    const response = await fetch(
      `/api/panel/recovery-packages/${packageId}/items/${itemId}/complete`,
      { method: "POST" },
    );
  ...
  async function checkpoint(
    packageId: string,
    responseValue: "NOT_YET" | "NEED_HELP" | "READY",
  ) {
    setBusy(packageId);
    const response = await fetch(
      `/api/panel/recovery-packages/${packageId}/checkpoint`,
      {
        method: "POST",
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {rows.map((row) => {
          const materials = row.items.filter(
            (item) => item.kind === "MATERIAL",
          );
          const assignments = row.items.filter(
