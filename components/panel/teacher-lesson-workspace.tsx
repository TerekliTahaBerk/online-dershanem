...
export function TeacherLessonWorkspace({
  ...
  const [showStudentExceptions, setShowStudentExceptions] = useState(
    !quickLessonCloseEnabled,
  );
  ...
  const exceptionCount = form.students.filter(
    (student) => student.attendance !== "PRESENT" || student.note.trim(),
  ).length;

  function recordInteraction() {
    if (!baselineMetricsEnabled) return;
    telemetry.current.interactionCount = Math.min(
      1000,
      telemetry.current.interactionCount + 1,
    );
    if (telemetry.current.startedAt !== null) return;
    telemetry.current.startedAt = performance.now();
    if (lesson.status === "COMPLETED") {
      sendPanelEvent({
        name: "lesson_close_reopened",
        properties: { groupSize: lesson.students.length },
      });
    } else {
      sendPanelEvent({
        name: "lesson_close_started",
        properties: {
          groupSize: lesson.students.length,
  ...
  function patchSharedForm(
    patch: Partial<
      Pick<LessonData, "topic" | "note" | "nextGoal" | "homework">
    >,
  ) {
    recordInteraction();
  ...
  const save = useCallback(
    async (complete: boolean) => {
      if (baselineMetricsEnabled && !complete)
        telemetry.current.draftSaveCount = Math.min(
          100,
          telemetry.current.draftSaveCount + 1,
        );
      setSaveState("saving");
      ...
      const requestBody = {
        ...
        outcomeSkipReason: learningOutcomesEnabled
          ? form.outcomeSkipReason
          : null,
        expectedVersion: quickLessonCloseEnabled
          ? closeVersion.current
          : undefined,
        idempotencyKey:
          complete && quickLessonCloseEnabled
            ? closeIdempotencyKey.current
            : undefined,
        assignmentDraft,
      };
      ...
      if (!saved && !queued && baselineMetricsEnabled) {
        sendPanelEvent({
          ...
          properties: {
            groupSize: lesson.students.length,
            completionAttempt: complete,
          },
        });
      }
      if (saved || queued) {
        lastSaved.current = noteSnapshot(form);
        if (saved && typeof responseBody?.version === "number")
          closeVersion.current = responseBody.version;
      }
      if (!saved && !queued && complete)
        setActionMessage(
          responseBody?.error ||
            "Ders kapatılamadı; değişiklikleriniz taslakta korunuyor.",
        );
      if (queued)
        setActionMessage(
          complete
      ...
      if (saved && complete) {
        ...
        if (baselineMetricsEnabled) {
          const initialStudents = new Map(
            lesson.students.map((student) => [student.id, student]),
          );
          const changedStudentCount = form.students.filter((student) => {
            const initial = initialStudents.get(student.id);
            return (
              !initial ||
              initial.note !== student.note ||
              initial.attendance !== student.attendance
            );
          }).length;
          sendPanelEvent({
            ...
            properties: {
              durationMs: Math.min(
                8 * 60 * 60 * 1000,
                Math.round(
                  telemetry.current.startedAt === null
                    ? 0
                    : performance.now() - telemetry.current.startedAt,
                ),
              ),
              groupSize: form.students.length,
              changedStudentCount,
              privateNoteCount: form.students.filter((student) =>
                student.note.trim(),
              ).length,
              filledSharedFieldCount: [
                form.topic,
                form.note,
                form.nextGoal,
                form.homework,
  ...
  useEffect(() => {
    ...
    const guardLink = (event: MouseEvent) => {
      ...
      if (
        !window.confirm(
          "Notlarda henüz güvenli biçimde kaydedilmemiş değişiklikler var. Yine de ayrılmak istiyor musunuz?",
        )
      ) {
        event.preventDefault();
  ...
  useEffect(() => {
    ...
    const synced = (event: Event) => {
      ...
      if (typeof detail.body?.version === "number")
        closeVersion.current = detail.body.version;
      setSaveState("saved");
      if (detail.requestBody?.complete) {
        setCompleted(true);
        setActionMessage("Cihazda bekleyen ders kapanışı güvenle eşitlendi.");
      } else
        setActionMessage("Cihazda bekleyen ders taslağı güvenle eşitlendi.");
    };
    const conflicted = (event: Event) => {
      const detail = (event as CustomEvent<{ kind: string; url: string }>)
        .detail;
      if (detail?.kind !== "LESSON_CLOSE" || detail.url !== url) return;
      setSaveState("error");
      setActionMessage(
        "Ders başka yerde değişti. Son kaydı açıp cihazdaki değişiklikleri yeniden uygulayın.",
      );
    };
  ...
  async function createAssignment() {
    if (!form.homework.trim())
      return setActionMessage("Önce çalışma alanını doldurun.");
    ...
    const response = await fetch("/api/panel/assignments", {
      ...
      body: JSON.stringify({
        ...
        description: form.homework,
        dueAt: new Date(Date.now() + 7 * 86400000).toISOString(),
        outcomeIds: learningOutcomesEnabled
          ? form.outcomeLinks.map((item) => item.outcomeId)
          : [],
        outcomeSkipReason: learningOutcomesEnabled
          ? form.outcomeSkipReason
          : null,
    ...
    setActionMessage(
      response.ok
        ? "Çalışma ödeve dönüştürüldü ve öğrencilere gönderildi."
        : "Ödev oluşturulamadı.",
    );
  }
  ...
  function toggleAssignmentPreview() {
    if (!form.homework.trim())
      return setActionMessage(
        "Ödev önizlemesi için önce çalışma alanını doldurun.",
      );
  ...
  function applyTemplate(
    template: Pick<NoteTemplate, "note" | "nextGoal" | "homework">,
  ) {
    recordInteraction();
  ...
  function usePreviousGoal() {
    ...
    setForm((current) => ({
      ...current,
      topic: current.topic || current.previousGoal || "",
    }));
  }

  async function saveTemplate() {
    if (templateTitle.trim().length < 2)
      return setActionMessage("Şablona kısa bir ad verin.");
    ...
    if (!response.ok)
      return setActionMessage(body.error || "Şablon kaydedilemedi.");
  ...
  async function deleteTemplate(id: string) {
    const response = await fetch(`/api/panel/teacher/templates/${id}`, {
      method: "DELETE",
    });
    if (response.ok)
      setTemplates((current) =>
      ...
      <section className="overflow-hidden rounded-[14px] border border-[var(--site-line)] bg-white shadow-[0_18px_55px_-35px_rgba(20,20,15,.28)]">
        ...
        <div className="space-y-6 p-5 sm:p-7">
            ...
            <button
              ...
              onClick={() =>
                applyTemplate({
                  ...
                  homework:
                    "Konu tarama çalışmasını tamamla ve yanlışlarını işaretle.",
                })
              }
              className="panel-quick-action"
            >
              <Sparkles size={14} /> Dengeli ders şablonu
            </button>
            <button
              type="button"
              onClick={() =>
                applyTemplate({
                  ...
                  homework:
                    "Kolay düzey 15 soru çöz; takıldığın soruları işaretle.",
                })
              }
              className="panel-quick-action"
            >
          ...
          {form.previousGoal ? (
            <button
              type="button"
              onClick={usePreviousGoal}
              className="group flex w-full items-start gap-3 rounded-2xl border border-[#eadf9e] bg-[#fff9dc] p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md"
            >
              <span className="rounded-xl bg-white p-2 text-amber-700">
                <Sparkles size={17} />
              </span>
              <span>
                <span className="block text-xs font-bold uppercase tracking-[.06em] text-amber-800">
                  Geçen dersten akıllı öneri
                </span>
          ...
          {quickLessonCloseEnabled ? (
            <div
              className="grid gap-3 rounded-2xl border border-[#dfe7c4] bg-[#f7f9ef] p-4 sm:grid-cols-3"
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1 line-clamp-2 text-sm">
                  {form.previousContext?.nextGoal ||
                    "İlk ders; önceki hedef yok"}
                </p>
              </div>
              <div>
                <p className="text-[10px] font-extrabold uppercase tracking-[.07em] text-[var(--brand-olive)]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="sm:col-span-2">
              ...
              <input
                value={form.topic}
                onChange={(event) =>
                  patchSharedForm({ topic: event.target.value })
                }
                className="panel-input mt-2 text-base font-semibold"
            ...
            <label>
              ...
              <textarea
                value={form.note}
                onChange={(event) =>
                  patchSharedForm({ note: event.target.value })
                }
            ...
            <label>
              ...
              <textarea
                value={form.nextGoal}
                onChange={(event) =>
                  patchSharedForm({ nextGoal: event.target.value })
                }
                className="panel-input mt-2 min-h-28 resize-y"
            ...
            <label className="sm:col-span-2">
              ...
              <input
                value={form.homework}
                onChange={(event) =>
                  patchSharedForm({ homework: event.target.value })
                }
          ...
          {learningOutcomesEnabled ? (
            <>
              <OutcomePicker
                outcomes={outcomes}
                value={form.outcomeLinks}
                onChange={(outcomeLinks) => {
                  recordInteraction();
                  setForm((current) => ({
                    ...current,
                    outcomeLinks,
                    outcomeSkipReason: outcomeLinks.length
                      ? null
                      : current.outcomeSkipReason,
                  }));
                }}
                withEvidence
              />
              {!form.outcomeLinks.length ? (
                <label className="block">
                  <span className="panel-label">
                    Kazanım seçmeden devam etme nedeni
                  </span>
                  <select
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        outcomeSkipReason: (event.target.value ||
                          null) as LessonData["outcomeSkipReason"],
                      }))
                    }
                    className="panel-input mt-2"
                  ...
                  >
                    ...
                    <option value="CATALOG_MISSING">
                      Katalogda uygun kazanım yok
                    </option>
                    <option value="NOT_APPLICABLE">
                      Bu ders için uygulanabilir değil
                    </option>
                  </select>
                </label>
              ) : null}
          ...
          {quickLessonCloseEnabled && assignmentPreview ? (
            <div className="rounded-2xl border border-violet-200 bg-violet-50/60 p-4">
              <div className="flex items-start gap-3">
                <Eye size={17} className="mt-0.5 text-violet-700" />
                <div>
                  <p className="text-sm font-extrabold text-violet-950">
                    Ödev önizlemesi
                  </p>
                  <p className="mt-1 text-xs leading-5 text-violet-900">
                    {form.homework} · 7 gün · yalnız seçilen öğrencilere
                  </p>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {form.students.map((student) => (
                  <label
                    key={student.id}
                    className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-bold"
                  >
                    <input
                      type="checkbox"
                      checked={assignmentRecipients.includes(student.id)}
                      onChange={() =>
                        setAssignmentRecipients((current) =>
              ...
              {!assignmentRecipients.length ? (
                <p className="mt-2 text-xs font-bold text-rose-700">
                  Göndermek için en az bir öğrenci seçin.
                </p>
              ) : null}
            </div>
          ) : null}
        ...
        {quickLessonCloseEnabled ? (
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="panel-quick-action justify-center"
            >
              <UsersRound size={14} /> Tümü burada
            </button>
            <button
              type="button"
              onClick={() => setShowStudentExceptions((current) => !current)}
              className="panel-quick-action justify-center"
            >
              {showStudentExceptions ? "İstisnaları gizle" : "İstisna ekle"}
            </button>
          </div>
        ) : null}
        {!quickLessonCloseEnabled ||
          ...
          form.students.map((student, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`grid h-9 w-9 place-items-center rounded-xl text-sm font-extrabold ${["bg-[#dceaf6] text-[#1e3a5f]", "bg-[#fcedb4] text-[#6b5310]", "bg-[#e6e0f0] text-[#3f3463]", "bg-[#d7e5d5] text-[#2f4a2a]"][index % 4]}`}
                >
              ...
              {student.supportLabels.length ? (
                <ul
                  aria-label={`${student.name} işlevsel destekleri`}
                  className="mt-2 flex flex-wrap gap-1"
                >
                  {student.supportLabels.map((label) => (
                    <li
                      key={label}
                      className="rounded-full bg-sky-50 px-2 py-1 text-[9px] font-bold text-sky-900"
                    >
                      {label}
                    </li>
                  ))}
              ...
              <textarea
                ...
                value={student.note}
                onChange={(event) =>
                  patchStudent(student.id, { note: event.target.value })
                }
                className="panel-input mt-3 min-h-20 resize-none text-xs"
