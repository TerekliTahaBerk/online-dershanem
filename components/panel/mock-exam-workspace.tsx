...
function initialSections(exam: CurriculumExam): SectionState[] {
...
export function MockExamWorkspace({
  ...
  async function submit() {
    ...
    try {
      ...
      setExams((current) => [body.exam as MockExamView, ...current]);
  ...
  return (
    <div className="space-y-6">
      {canCreate ? (
        <section className="panel-surface p-5" onFocus={markStarted}>
              ...
              {sections.map((section, index) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(mockExamErrorLabels).map(
                      ([value, label]) => (
                        <label
                          key={value}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            >
              <Save size={15} /> {saving ? "Kaydediliyor…" : "Denemeyi kaydet"}
            </button>
            {previous ? (
              <button
                type="button"
              ...
              >
                <RotateCcw size={14} /> Son değişikliği geri al
              </button>
            ) : null}
            {status ? (
              <p
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        <article className="panel-surface p-5">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-[620px] w-full text-xs">
              <thead>
                <tr>
                  <th className="pb-2 text-left">Bölüm</th>
        ...
        <article className="panel-surface p-5">
          <h2 className="text-sm font-extrabold">Sonraki küçük eylem</h2>
          {trend.recurringError ? (
            <div className="mt-4 rounded-2xl bg-[#fff9dc] p-4">
              <p className="text-[10px] font-extrabold uppercase text-amber-800">
                {mockExamErrorLabels[trend.recurringError.category]} · son 3
                denemede {trend.recurringError.count} işaret
              </p>
              <p className="mt-2 text-xs font-bold leading-5 text-amber-950">
                {trend.recurringError.action}
              </p>
              {canReview && visibleExams[0] && !visibleExams[0].nextAction ? (
                <button
                  type="button"
                  onClick={() =>
                    void approveAction(
                  ...
                  className="mt-3 rounded-xl bg-amber-900 px-3 py-2 text-[10.5px] font-bold text-white"
                >
                  <CheckCircle2 size={13} className="mr-1 inline" /> Öğretmen
                  olarak onayla
                </button>
              ) : null}
            </div>
      ...
      <section className="panel-surface p-5">
        <h2 className="text-sm font-extrabold">Son denemeler</h2>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {visibleExams.map((exam) => {
            const net = exam.sections.reduce(
              (sum, section) =>
                sum +
                sectionNet(
                  exam.exam,
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-1 text-xs font-bold">
                ...
                <p className="mt-3 text-[10.5px] text-[var(--site-muted)]">
                  ...
                  doğru ·{" "}
                  {exam.sections.reduce(
                    (sum, section) => sum + section.incorrectCount,
                    0,
                  )}{" "}
                  yanlış ·{" "}
                  {exam.sections.reduce(
                    (sum, section) => sum + section.blankCount,
                    0,
                  )}{" "}
                  boş
                ...
                {exam.nextAction ? (
                  <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-[10.5px] font-bold leading-5 text-emerald-900">
                ...
                {role === "STUDENT" || canReview ? (
                  <ReasonEditor
                    ...
                    onSaved={(sections) =>
                      setExams((current) =>
                        current.map((item) =>
                          item.id === exam.id ? { ...item, sections } : item,
                        ),
                      )
                    }
                  />
  ...
  return (
    <details className="mt-3 border-t border-[var(--site-line)] pt-3">
      ...
      <button
        type="button"
        onClick={() => void save()}
        className="panel-secondary-button mt-3"
      >
        Nedenleri kaydet
      </button>
      {message ? (
        <p role="status" className="mt-2 text-[10.5px] font-bold">
          {message}
        </p>
      ) : null}
    </details>
