...
export function TeacherDigestReview({ initialRows }: { initialRows: Row[] }) {
  const [rows, setRows] = useState(initialRows);
  const [message, setMessage] = useState("");
  async function generate(studentId: string) {
    const response = await fetch("/api/panel/weekly-digests/generate", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ studentId }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) return setMessage(body.error || "Taslak hazırlanamadı.");
    window.location.reload();
  }
  async function publish(row: Row) {
    if (!row.digest) return;
    const response = await fetch(
      `/api/panel/weekly-digests/${row.digest.id}/publish`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ expectedVersion: row.digest.version }),
      },
    );
    const body = await response.json().catch(() => ({}));
    if (!response.ok) return setMessage(body.error || "Özet yayınlanamadı.");
    setRows((current) =>
      current.map((item) =>
        item.studentId === row.studentId && item.digest
          ? {
              ...item,
              digest: {
                ...item.digest,
                status: "PUBLISHED",
                version: item.digest.version + 1,
              },
            }
          : item,
      ),
    );
    setMessage(`${row.name} için aynı özet öğrenci ve veliye açıldı.`);
  }
  return (
          ...
          {row.digest ? (
            <div className="mt-4 rounded-2xl border border-[var(--site-line)] bg-[var(--site-bg-warm)] p-4">
              ...
              <ul className="mt-3 space-y-2 text-xs leading-5">
                <li>✓ {row.digest.goodThingOne}</li>
                <li>✓ {row.digest.goodThingTwo}</li>
                <li>→ {row.digest.supportArea}</li>
                <li>?</li>
                <li>{row.digest.homeQuestion}</li>
              </ul>
              {row.digest.status === "DRAFT" ? (
                <button
                  type="button"
