...
const statusLabels = {
  ...
  SNOOZED: "Bekletildi",
  RESOLVED: "Çözüldü",
  FALSE_POSITIVE: "Yanlış işaret",
} as const;
const outcomeLabels: Record<string, string> = {
  CHECK_IN_COMPLETED: "Öğrenciyle kısa görüşme yapıldı",
  SUPPORT_PLANNED: "Destek adımı planlandı",
  PRACTICE_ADJUSTED: "Çalışma yeniden düzenlendi",
  FAMILY_CONTACTED: "Aileyle iletişim kuruldu",
  NO_ACTION_NEEDED: "Ek işlem gerekmedi",
  OTHER: "Başka kontrollü sonuç",
};
const activityLabels: Record<string, string> = {
  GENERATED: "Kural sinyali oluşturdu",
  ASSIGNED: "Sahiplenildi",
  STARTED: "İnceleme başladı",
  ACTION_LOGGED: "İnsan aksiyonu kaydedildi",
  SNOOZED: "Bekletildi",
  RESOLVED: "Çözüldü",
  FALSE_POSITIVE: "Yanlış işaret olarak kapandı",
...
export function InterventionInbox({ rows }: { rows: Row[] }) {
  ...
  const openCount = rows.filter(
    (row) => !["RESOLVED", "FALSE_POSITIVE"].includes(row.status),
  ).length;
  const overdueCount = rows.filter(
    (row) =>
      !["RESOLVED", "FALSE_POSITIVE", "SNOOZED"].includes(row.status) &&
      new Date(row.dueAt) < new Date(),
  ).length;
  const falsePositiveCount = rows.filter(
    (row) => row.status === "FALSE_POSITIVE",
  ).length;
  ...
  async function generate() {
    setBusy("generate");
    setMessage("");
    const response = await fetch("/api/panel/interventions/generate", {
      method: "POST",
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setMessage(body.error || "Sinyaller yenilenemedi.");
      setBusy(null);
      return;
    }
    setMessage(
      `${body.createdCount} yeni açıklanabilir kayıt eklendi${body.reactivatedCount ? `, ${body.reactivatedCount} bekletilen kayıt yeniden açıldı` : ""}.`,
    );
  ...
  async function act(
    ...
    setBusy(row.id);
    setMessage("");
    const response = await fetch(`/api/panel/interventions/${row.id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ action, expectedVersion: row.version, ...extra }),
    });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      setMessage(body.error || "İşlem kaydedilemedi.");
      setBusy(null);
      return;
    }
    window.location.reload();
      ...
      <div className="mt-5 space-y-4">
        {visible.length ? (
          visible.map((row) => {
            ...
            const disabled = !hydrated || busy !== null || !row.canAct;
            return (
              <article
                key={row.id}
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[var(--site-muted)]">
                  ...
                  {row.snoozedUntil ? (
                    <span>
                      Bekletme sonu:{" "}
                      <strong>
                        {new Date(row.snoozedUntil).toLocaleDateString("tr-TR")}
                      </strong>
                    </span>
                  ) : null}
                </div>
                {!row.canAct ? (
                  <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-xs font-bold text-amber-900">
                    Bu kaydı başka bir ekip üyesi sahiplenmiş. Geçmiş görünür;
                    değişiklik yalnız sahibi veya admin yapabilir.
                  </p>
                ) : null}
                {!closed && row.canAct ? (
                  <div className="mt-5 border-t border-[var(--site-line)] pt-5">
                    ...
                    <label className="mt-4 block text-xs font-bold">
                      ...
                      <textarea
                        aria-label={`${row.studentName} için iç aksiyon notu`}
                        value={notes[row.id] || ""}
                        onChange={(event) =>
                          setNotes((current) => ({
                            ...current,
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        disabled={disabled || !(notes[row.id] || "").trim()}
                        onClick={() =>
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      <label className="text-xs font-bold">
                        Sonuç
                        <select
                          aria-label={`${row.studentName} sonuç`}
                          value={outcomes[row.id] || "SUPPORT_PLANNED"}
                          onChange={(event) =>
                            setOutcomes((current) => ({
                              ...current,
                              [row.id]: event.target.value,
                        ...
                        >
                          ...
                          <option value="SUPPORT_PLANNED">
                            Destek adımı planlandı
                          </option>
                          <option value="PRACTICE_ADJUSTED">
                            Çalışma yeniden düzenlendi
                          </option>
                          <option value="FAMILY_CONTACTED">
                            Aileyle iletişim kuruldu
                          </option>
                          <option value="NO_ACTION_NEEDED">
                            Ek işlem gerekmedi
                          </option>
                          <option value="OTHER">Başka kontrollü sonuç</option>
                        </select>
                        <button
                          type="button"
                          disabled={disabled}
                          onClick={() =>
                            act(row, "RESOLVE", {
                              outcomeCode:
                                outcomes[row.id] || "SUPPORT_PLANNED",
                              note: notes[row.id] || undefined,
                            })
                          }
                      ...
                      <label className="text-xs font-bold">
                        Yanlış işaret nedeni
                        <select
                          aria-label={`${row.studentName} yanlış işaret nedeni`}
                          value={falseReasons[row.id] || "CONTEXT_MISSING"}
                          onChange={(event) =>
                            setFalseReasons((current) => ({
                              ...current,
                              [row.id]: event.target.value,
                            }))
                          }
                        ...
                        >
                          ...
                          <option value="THRESHOLD_TOO_SENSITIVE">
                            Eşik fazla hassastı
                          </option>
                          <option value="DUPLICATE">Tekrarlayan kayıttı</option>
                          <option value="OTHER">Başka kontrollü neden</option>
                        </select>
                        <button
                          type="button"
                          disabled={disabled}
                          onClick={() =>
                            act(row, "FALSE_POSITIVE", {
                              falsePositiveReason:
                                falseReasons[row.id] || "CONTEXT_MISSING",
                              note: notes[row.id] || undefined,
                            })
                          }
                          className="panel-quick-action mt-2"
