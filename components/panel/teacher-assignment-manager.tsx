...
type Assignment = { id: string; groupId: string; groupName: string; title: string; description: string; dueAt: string; isActive: boolean; done: number; total: number };
...
export function TeacherAssignmentManager({ groups, lessons, assignments }: { groups: Group[]; lessons: Lesson[]; assignments: Assignment[] }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
    <form className="panel-surface h-fit p-5" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void run("create", () => request("/api/panel/assignments", "POST", { groupId: data.get("groupId"), lessonId: data.get("lessonId") || null, title: data.get("title"), description: data.get("description"), dueAt: new Date(String(data.get("dueAt"))).toISOString() }), "Ödev öğrencilere gönderildi."); }}>
