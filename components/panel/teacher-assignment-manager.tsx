...
function AssignmentOutcomeBackfill({
  assignment,
  outcomes,
  onSaved,
}: {
  ...
  const [reason, setReason] = useState<
    "CATALOG_MISSING" | "COMPLETE_LATER" | "NOT_APPLICABLE" | ""
  >("COMPLETE_LATER");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  return (
    <details className="mt-3 rounded-xl border border-dashed border-amber-300 bg-amber-50/60 p-3">
      <summary className="cursor-pointer text-[10.5px] font-extrabold text-amber-900">
        Kazanım etiketi bekliyor · tamamla
      </summary>
      <div className="mt-3">
        <OutcomePicker
          outcomes={outcomes}
        ...
        {!selected.length ? (
          ...
          >
            ...
            <option value="NOT_APPLICABLE">Uygulanabilir değil</option>
          </select>
        ) : null}
        <button
          type="button"
          ...
          onClick={() => {
            setBusy(true);
            setError("");
            void request(`/api/panel/assignments/${assignment.id}`, "PATCH", {
              title: assignment.title,
              description: assignment.description,
              dueAt: assignment.dueAt,
              isActive: assignment.isActive,
              outcomeIds: selected.map((item) => item.outcomeId),
              outcomeSkipReason: reason || null,
            })
              .then(onSaved)
              .catch((caught) =>
                setError(
                  caught instanceof Error ? caught.message : "Kaydedilemedi.",
                ),
              )
...
function SubmissionReview({
  ...
  const [scores, setScores] = useState<
    Record<string, "NEEDS_WORK" | "DEVELOPING" | "MEETS">
  >({});
  const [busy, setBusy] = useState(false);
  ...
  async function review(decision: "APPROVE" | "REQUEST_CHANGES") {
    ...
    const response = await fetch(
      `/api/panel/assignment-submissions/${submission.id}/review`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          ...
          decision,
          feedback,
          interactionDurationMs: Date.now() - openedAt,
          scores: assignment.criteria!.map((criterion) => ({
            criterionId: criterion.id,
            level: scores[criterion.id],
          })),
        }),
      },
    );
    ...
    if (!response.ok) {
      setMessage(body.error || "Değerlendirme kaydedilemedi.");
      setBusy(false);
      return;
    }
  ...
  return (
    <article className="mt-3 rounded-2xl border border-[var(--site-line)] bg-[var(--site-bg-warm)] p-4">
      ...
      <p className="mt-3 rounded-xl bg-white p-3 text-xs leading-5">
        {submission.textEvidence}
      </p>
      <div className="mt-3 space-y-2">
        {assignment.criteria.map((criterion) => (
          <label
            key={criterion.id}
            className="grid gap-1 text-[11px] font-bold sm:grid-cols-[1fr_150px] sm:items-center"
          >
            <span>{criterion.label}</span>
            <select
              aria-label={`${submission.studentName} ${criterion.label}`}
              value={scores[criterion.id] || ""}
              onChange={(event) =>
                setScores((current) => ({
                  ...current,
                  [criterion.id]: event.target.value as
                    | "NEEDS_WORK"
                    | "DEVELOPING"
                    | "MEETS",
                }))
            ...
            >
              ...
              <option value="DEVELOPING">Gelişiyor</option>
              <option value="MEETS">Karşılıyor</option>
            </select>
          </label>
        ))}
      </div>
      <textarea
        aria-label={`${submission.studentName} geri bildirim`}
        value={feedback}
      ...
      {message ? (
        <p className="mt-2 text-xs font-bold text-rose-700">{message}</p>
      ) : null}
    </article>
  );
}
...
export function TeacherAssignmentManager({
  ...
  assignmentEvidenceEnabled = false,
}: {
  groups: Group[];
  lessons: Lesson[];
  ...
  assignmentEvidenceEnabled?: boolean;
}) {
  ...
  const [outcomeSkipReason, setOutcomeSkipReason] = useState<
    "CATALOG_MISSING" | "COMPLETE_LATER" | "NOT_APPLICABLE" | ""
  >("");
  const [evidenceRequired, setEvidenceRequired] = useState(false);
  const [criteria, setCriteria] = useState([
    "Çözüm yolunu açıkça gösterir",
    "Sonucunu kontrol eder",
  ]);
  const [items, setItems] = useState(assignments);
  const groupLessons = useMemo(
    () => lessons.filter((lesson) => lesson.groupId === groupId),
    [groupId, lessons],
  );
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          void run(
            "create",
            () =>
              request("/api/panel/assignments", "POST", {
                groupId: data.get("groupId"),
                lessonId: data.get("lessonId") || null,
                title: data.get("title"),
                description: data.get("description"),
                dueAt: new Date(String(data.get("dueAt"))).toISOString(),
                outcomeIds: selectedOutcomes.map((item) => item.outcomeId),
                outcomeSkipReason: outcomeSkipReason || null,
                evidenceRequired,
                rubricCriteria: evidenceRequired ? criteria : [],
              }),
            "Ödev öğrencilere gönderildi.",
          );
        }}
      >
                ...
                <div className="flex gap-2">
                  ...
                  {criteria.length > 2 ? (
                    <button
                      type="button"
                      onClick={() =>
                        setCriteria((current) => current.slice(0, -1))
                      }
          ...
          <div className="mt-3">
            <OutcomePicker
              ...
              onChange={(value) => {
                ...
                if (value.length) setOutcomeSkipReason("");
              }}
            />
            {!selectedOutcomes.length ? (
              <select
                value={outcomeSkipReason}
                onChange={(event) =>
              ...
              >
                ...
                <option value="NOT_APPLICABLE">
                  Bu ödev için uygulanabilir değil
                </option>
              </select>
            ) : null}
          </div>
      ...
      <section>
        ...
        <div className="mt-3 space-y-3">
          {items.map((assignment) => {
            const percent = assignment.total
              ? Math.round((assignment.done / assignment.total) * 100)
              : 0;
            return (
              <article
                key={assignment.id}
              ...
              >
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    {assignment.outcomes?.length ? (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {assignment.outcomes.map((outcome) => (
                          <span
                            key={outcome}
                            className="rounded-full bg-violet-50 px-2 py-1 text-[9px] font-bold text-violet-700"
                          >
                            {outcome}
                          </span>
                        ))}
                    ...
                    {learningOutcomesEnabled && !assignment.outcomes?.length ? (
                      <AssignmentOutcomeBackfill
                        assignment={assignment}
                        outcomes={outcomes}
                  ...
                  <div className="shrink-0 sm:w-40">
                    ...
                    <div
                      ...
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-label={`${assignment.title} tamamlanma oranı`}
                    >
                      <div
                    ...
                    {assignment.isActive ? (
                      <button
                        ...
                        onClick={() =>
                          void run(
                            ...
                            () =>
                              request(
                                `/api/panel/assignments/${assignment.id}`,
                                "PATCH",
                                {
                                  title: assignment.title,
                                  description: assignment.description,
                      ...
                      <span className="mt-3 flex items-center gap-1 text-[10.5px] font-bold text-emerald-700">
                        <CheckCircle2 size={12} /> Kayıt korunuyor
                      </span>
                    )}
                  </div>
                </div>
        ...
        items.some((assignment) =>
          ...
          <div className="mt-6">
            <h2 className="text-sm font-extrabold">
              Değerlendirme bekleyen kanıtlar
            </h2>
