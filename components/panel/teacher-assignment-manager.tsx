...
function SubmissionReview({ assignment, submission, onSaved }: { assignment: Assignment; submission: Submission; onSaved: () => void }) {
...
export function TeacherAssignmentManager({ groups, lessons, assignments, outcomes = [], learningOutcomesEnabled = false, assignmentEvidenceEnabled = false }: { groups: Group[]; lessons: Lesson[]; assignments: Assignment[]; outcomes?: OutcomeOption[]; learningOutcomesEnabled?: boolean; assignmentEvidenceEnabled?: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
    ...
    <section>
      ...
      {assignmentEvidenceEnabled && assignments.some((assignment) => assignment.submissions.some((submission) => submission.status === "SUBMITTED")) ? <div className="mt-6"><h2 className="text-sm font-extrabold">Değerlendirme bekleyen kanıtlar</h2>{assignments.flatMap((assignment) => assignment.submissions.filter((submission) => submission.status === "SUBMITTED").map((submission) => <SubmissionReview key={submission.id} assignment={assignment} submission={submission} onSaved={() => router.refresh()} />))}</div> : null}
    </section>
  </div>;
}
