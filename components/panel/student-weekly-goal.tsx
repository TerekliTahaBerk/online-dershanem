...
export function StudentWeeklyGoal({ initial }: { initial: string }) {
  ...
  return (
    <section className="rounded-[14px] border border-[#eadf9e] bg-[#fff9dc] p-5">
      ...
      {editing ? (
        <form
          ...
          onSubmit={async (event) => {
            ...
            setBusy(true);
            setMessage("");
            const response = await fetch("/api/panel/student/weekly-goal", {
              method: "PATCH",
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            <span className="text-[10.5px] text-amber-900/65">
              {draft.length}/180
            </span>
            <button
      ...
      <p aria-live="polite" className="mt-2 text-xs font-bold text-emerald-700">
        {message}
      </p>
    </section>
  );
