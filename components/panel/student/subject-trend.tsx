...
export type SubjectSeries = { name: string; color: string; nets: number[] };
...
export function SubjectTrendCard({
  ...
  const all = series.flatMap((s) => s.nets);
  ...
  const toPoints = (nets: number[]) =>
    nets
      .map((n, i) => `${Math.round(i * step)},${Math.round(H - 30 - ((n - min) / span) * (H - 60))}`)
      .join(" ");
