      ...
      {adminLayout ? (
        <div className="min-h-dvh lg:grid lg:grid-cols-[252px_minmax(0,1fr)]">
          <aside className="sticky top-0 hidden h-dvh flex-col border-r border-[var(--site-line)] bg-[#f7f6f0]/92 px-4 py-5 backdrop-blur-xl lg:flex">
            <Link href="/panel/yonetim" className="flex items-center gap-3 px-2 text-[14px] font-bold tracking-[-.025em] text-[var(--site-ink)]">
              <span className="grid h-10 w-10 place-items-center rounded-[14px] bg-[var(--brand-olive)] text-[13px] font-black text-white shadow-[0_12px_28px_-13px_rgba(58,74,44,.85)]">od</span>
              <span><span className="block">onlinedershanem</span><span className="mt-0.5 block text-[10px] font-semibold uppercase tracking-[.09em] text-[var(--site-muted)]">Yönetim alanı</span></span>
            </Link>
          ...
          <div className="min-w-0">
            <header className="sticky top-0 z-40 border-b border-[var(--site-line)] bg-[#fbfaf5]/88 backdrop-blur-xl">
              <div className="flex h-[68px] items-center justify-between gap-3 px-4 sm:px-6 xl:px-8">
                <div className="flex min-w-0 items-center gap-3 lg:hidden"><Link href="/panel/yonetim" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[var(--brand-olive)] text-[12px] font-black text-white">od</Link><span className="truncate text-xs font-bold text-[var(--site-ink)]">Yönetim</span></div>
        ...
        <>
          <header className="sticky top-0 z-40 border-b border-[var(--site-line)] bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-[68px] max-w-[1320px] items-center justify-between gap-4 px-4 sm:px-6"><div className="flex items-center gap-3"><Link href="/panel" className="flex items-center gap-2.5 text-[14px] font-bold tracking-[-.02em] text-[var(--site-ink)]"><span className="grid h-8 w-8 place-items-center rounded-xl bg-[var(--brand-olive)] text-[13px] font-black text-white">od</span><span className="hidden sm:inline">onlinedershanem</span></Link><span className="rounded-full bg-[var(--brand-olive-soft)] px-2.5 py-1 text-[10.5px] font-bold uppercase tracking-[.06em] text-[var(--brand-olive)]">{roleLabel(role)}</span></div><div className="flex items-center gap-3"><span className="hidden text-[13px] text-[var(--site-body)] sm:inline">{fullName || email}</span><LogoutButton /></div></div></header>
