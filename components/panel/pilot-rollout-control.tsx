...
type Cohort = {
  ...
  version: number;
  memberCount: number;
  coverage: { ADMIN: number; TEACHER: number; STUDENT: number; PARENT: number };
  createdAt: string;
};

export function PilotRolloutControl({
  groups,
  cohorts,
}: {
  groups: Array<{ id: string; name: string; detail: string }>;
  cohorts: Cohort[];
}) {
  ...
  useEffect(() => setRows(cohorts), [cohorts]);
  async function create() {
    setBusy(true);
    setMessage("");
    const response = await fetch("/api/panel/pilot-cohorts", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ groupId, requestKey: crypto.randomUUID() }),
    });
    const body = await response.json().catch(() => ({}));
    setBusy(false);
    if (!response.ok)
      return setMessage(body.error || "Pilot taslağı oluşturulamadı.");
    const created = body.cohort;
    const coverage = created.members.reduce(
      (
        counts: Cohort["coverage"],
        member: { role: keyof Cohort["coverage"] },
    ...
    setRows((current) => [
      {
        id: created.id,
        groupName: created.groupNameSnapshot,
    ...
    setMessage(
      "Dört rollü pilot taslağı oluşturuldu; aktivasyon kapılarını kontrol edin.",
    );
    router.refresh();
  }
  async function transition(
    cohort: Cohort,
    action: "ACTIVATE" | "PAUSE" | "RESUME" | "COMPLETE" | "ROLLBACK",
  ) {
    setBusy(true);
    setMessage("");
    const response = await fetch(`/api/panel/pilot-cohorts/${cohort.id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        action,
        expectedVersion: cohort.version,
        stopReason: ["PAUSE", "ROLLBACK"].includes(action)
          ? stopReason
          : undefined,
      }),
    });
    ...
    setBusy(false);
    if (!response.ok)
      return setMessage(body.error || "Pilot durumu güncellenemedi.");
    setRows((current) =>
      current.map((item) =>
        item.id === cohort.id
          ? {
              ...item,
              status: body.cohort.status,
              version: body.cohort.version,
            }
          : item,
      ),
    );
    setMessage(
      action === "ACTIVATE" || action === "RESUME"
        ? "Pilot erişimi etkinleştirildi."
        : action === "COMPLETE"
          ? "Pilot tamamlandı; kayıtlar korundu."
          : "Pilot erişimi güvenle durduruldu; kayıtlar korunuyor.",
    );
    router.refresh();
  }
  return (
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <label className="text-xs font-extrabold">
            ...
            >
              ...
              <option value="GUARDRAIL_BREACH">Guardrail ihlali</option>
              <option value="SECURITY_INCIDENT">Güvenlik olayı</option>
              <option value="DATA_QUALITY">Veri kalitesi</option>
            </select>
          </label>
        </div>
        {message ? (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${cohort.status === "ACTIVE" ? "bg-emerald-100 text-emerald-800" : cohort.status === "PAUSED" ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-700"}`}
                    >
                      {cohort.status}
                    </span>
                  </div>
