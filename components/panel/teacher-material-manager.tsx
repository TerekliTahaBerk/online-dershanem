...
export function TeacherMaterialManager({
  groups,
  materials,
}: {
  groups: Group[];
  ...
  async function archive(id: string) {
    ...
    setArchiveBusy(id);
    const response = await fetch(`/api/panel/materials/${id}`, {
      method: "PATCH",
    });
    if (response.ok) {
      setItems((current) =>
      ...
      router.refresh();
    } else setMessage("Materyal arşivlenemedi.");
    setArchiveBusy(null);
  }
    ...
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <form
        ...
        onSubmit={async (event) => {
          ...
          const response =
            source === "FILE"
              ? await fetch("/api/panel/materials/upload", {
                  method: "POST",
                  body: data,
                })
              : await fetch("/api/panel/materials", {
                  method: "POST",
                  ...
                  body: JSON.stringify({
                    ...
                    url: data.get("url"),
                    kind: data.get("kind"),
                    captionsAvailable: data.get("captionsAvailable") === "on",
                    transcript: data.get("transcript"),
                  }),
                });
          const result = await response.json().catch(() => ({}));
          if (response.ok) {
            setItems((current) => [result.material as Material, ...current]);
            formElement.reset();
            setMessage("Materyal öğrencilere açıldı.");
            setBusy(false);
            router.refresh();
          } else {
            setMessage(result.error || "Paylaşılamadı.");
            setBusy(false);
          }
        }}
      >
