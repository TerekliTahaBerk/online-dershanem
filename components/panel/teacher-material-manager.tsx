...
import { Archive, ExternalLink, FileText, Link2, Video } from "lucide-react";
...
export function TeacherMaterialManager({ groups, materials }: { groups: Group[]; materials: Material[] }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
    <form className="panel-surface h-fit p-5" onSubmit={async (event) => {
      ...
      const response = await fetch("/api/panel/materials", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ groupId: data.get("groupId"), title: data.get("title"), description: data.get("description"), url: data.get("url"), kind: data.get("kind") }) });
      ...
      <select name="kind" defaultValue="LINK" className="panel-input mt-2"><option value="LINK">Bağlantı</option><option value="PDF">PDF</option><option value="VIDEO">Video</option></select>
      <input name="title" required className="panel-input mt-2" placeholder="Materyal başlığı" />
      <input name="url" type="url" required className="panel-input mt-2" placeholder="https://..." />
      <textarea name="description" className="panel-input mt-2 min-h-24" placeholder="Kısa kullanım yönlendirmesi" />
      <button disabled={busy} className="site-btn site-btn-primary site-btn-sm mt-4 w-full">{busy ? "Paylaşılıyor" : "Materyali paylaş"}</button>
