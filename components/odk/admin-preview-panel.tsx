...
export function AdminPreviewPanel({ examId }: { examId: string }) {
  const [kind, setKind] =
    useState<(typeof kinds)[number]["id"]>("STUDENT_EXAM");
  ...
  async function load(nextKind = kind) {
    ...
    try {
      const response = await fetch(
        `/api/odk/admin/exams/${examId}/preview?kind=${nextKind}`,
      );
        ...
        <div className="mt-4 rounded-2xl border border-dashed border-[var(--site-line)] bg-[var(--site-bg-warm)] p-4">
          ...
          {preview.studentExam ? (
            <div className="mt-3 space-y-2 text-xs">
              <p>
                <strong>{preview.studentExam.questionCount}</strong> soru ·{" "}
                {preview.studentExam.durationMinutes} dk
              </p>
              <p>
                {preview.studentExam.sections
                  .map(
              ...
              <ul className="space-y-1">
                {preview.studentExam.securityNotes.map((note) => (
                  <li key={note}>• {note}</li>
                ))}
              </ul>
          ...
          {preview.teacherReport ? (
            <div className="mt-3 space-y-2 text-xs">
              <p>
                ...
                {preview.teacherReport.visibleFields.join(", ")}
              </p>
              <p>
                <strong>Gizli:</strong>{" "}
                {preview.teacherReport.hiddenFields.join(", ")}
              </p>
          ...
          {preview.parentReport ? (
            <div className="mt-3 space-y-2 text-xs">
              <p>
                <strong>Görünür:</strong>{" "}
                {preview.parentReport.visibleFields.join(", ")}
              </p>
              <p>
                <strong>Gizli:</strong>{" "}
                {preview.parentReport.hiddenFields.join(", ")}
              </p>
