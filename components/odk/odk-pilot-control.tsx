...
export function OdkPilotControl({
  candidates,
  runs,
  currentAdminId,
}: {
  ...
  const groups = useMemo(
    () =>
      roles.map((role) => ({
        role,
        users: candidates.filter((candidate) => candidate.role === role),
      })),
    [candidates],
  );
      ...
      roles.reduce(
        (coverage, role) => ({
          ...
          [role]: candidates.filter(
            (candidate) =>
              candidate.role === role && selected.includes(candidate.id),
          ).length,
        }),
        { ADMIN: 0, TEACHER: 0, STUDENT: 0, PARENT: 0 } as Record<Role, number>,
      ),
    [candidates, selected],
  );
  const missingRoles = roles.filter((role) => selectedCoverage[role] === 0);
  const needsSecondStudent = selectedCoverage.STUDENT < 2;
  const hasOpenRun = rows.some((run) =>
  ...
  async function create() {
    ...
    setMessage(null);
    try {
      const response = await fetch("/api/odk/admin/pilot-runs", {
        method: "POST",
      ...
      const run = body.run;
      const coverage = run.members.reduce(
        (counts: Record<Role, number>, member: { role: Role }) => ({
          ...counts,
      ...
      setMessage({
        text: "Bağlantı kurulamadı. Pilot taslağı oluşturulmadı; tekrar deneyin.",
        error: true,
      });
    } finally {
      setBusy(false);
  ...
  async function transition(
    ...
    const confirmations: Partial<Record<typeof action, string>> = {
      ...
      COMPLETE:
        "Pilot tamamlanacak ve tekrar açılamayacak. Audit kayıtları korunacak. Devam edilsin mi?",
      ROLLBACK:
        "Pilot geri alınacak ve terminal duruma geçecek. Sonuçlar açıklanmayacak; veriler korunacak. Devam edilsin mi?",
    };
    if (confirmations[action] && !window.confirm(confirmations[action])) return;
    setBusy(true);
    setMessage(null);
    try {
      const response = await fetch(`/api/odk/admin/pilot-runs/${run.id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          action,
      ...
      if (!response.ok)
        return setMessage({
          text: body.error || "Pilot durumu güncellenemedi.",
          error: true,
        });
      setRows((current) =>
        current.map((item) =>
          item.id === run.id
            ? { ...item, status: body.run.status, version: body.run.version }
            : item,
        ),
      );
      setMessage({
      ...
      setMessage({
        text: "Bağlantı kurulamadı. Pilot durumu değiştirilmedi; tekrar deneyin.",
        error: true,
      });
    } finally {
      setBusy(false);
        ...
        <div className="mt-4 grid grid-cols-4 gap-1.5">
          {roles.map((role) => (
            ...
            >
              ...
              <span className="text-[9px] font-bold">{roleLabels[role]}</span>
            </div>
          ))}
        </div>
          ...
          {groups.map((group) => (
            <fieldset key={group.role}>
              ...
              <div className="mt-2 space-y-1">
                ...
                {!group.users.length ? (
                  <p className="text-[11px] text-rose-700">Uygun hesap yok.</p>
                ) : null}
              </div>
            </fieldset>
          ))}
        ...
        {missingRoles.length || needsSecondStudent ? (
          ...
          >
            {missingRoles.length
              ...
              : ""}
            {needsSecondStudent ? "Pilot için en az iki öğrenci seçin." : ""}
          </p>
        ) : null}
        {hasOpenRun ? (
          <p
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-extrabold">{run.name}</h3>
                      <OdkStatusBadge
