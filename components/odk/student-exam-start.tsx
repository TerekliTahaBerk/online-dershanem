...
export function StudentExamStart({ examId, meetRequired, meetUrl, canStart, startError, activeAttempt }: { examId: string; meetRequired: boolean; meetUrl: string | null; canStart: boolean; startError: string | null; activeAttempt: boolean }) {
  ...
  return <div className="space-y-3">
    ...
    <section className="rounded-2xl border border-[var(--site-line)] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[var(--panel-nav-active)] text-xs font-extrabold text-[var(--brand-olive)]">{meetRequired ? "2" : "1"}</span><h2 className="font-bold text-[var(--site-ink)]">{activeAttempt ? "Denemeye devam et" : "Denemeyi başlat"}</h2></div><p className="mt-3 text-sm leading-6 text-[var(--site-body)]">{activeAttempt ? "Devam eden oturumun ve kalan süren korunuyor." : "Başlattığında kişisel sayacın çalışır. Başlatmadan önce cihazının şarjını ve bağlantını kontrol et."}</p><button type="button" onClick={start} disabled={busy || (!activeAttempt && (!canStart || !acknowledged))} className="panel-primary-button mt-4 w-full sm:w-auto">{busy ? <Loader2 size={17} className="animate-spin" /> : <PlayCircle size={17} />}{activeAttempt ? "Denemeye devam et" : "Denemeyi başlat"}</button>{!activeAttempt && startError ? <p className="mt-3 rounded-xl bg-[var(--pd-pastel-yellow-soft)] p-3 text-sm font-semibold text-[var(--pd-pastel-yellow-ink)]">{startError}</p> : null}{error ? <p role="alert" className="mt-3 rounded-xl bg-[var(--pd-pastel-blush-soft)] p-3 text-sm font-semibold text-[var(--pd-pastel-blush-ink)]">{error}</p> : null}</section>
  </div>;
}
