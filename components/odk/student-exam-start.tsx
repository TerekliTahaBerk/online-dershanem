...
export function StudentExamStart({
  ...
  async function start() {
    ...
    try {
      ...
      if (!response.ok)
        return setError(result.error || "Sınav oturumu başlatılamadı.");
      router.push(`/panel/odk/ogrenci/denemeler/${examId}/coz`);
      router.refresh();
    } catch {
      setError(
        "Bağlantı kurulamadı. Sınav oturumu başlatılmadı; tekrar deneyin.",
      );
    } finally {
      setBusy(false);
    }
    ...
    <div className="space-y-3">
      <div className="rounded-2xl border border-[var(--site-line)] bg-[var(--site-bg-warm)] p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold text-[var(--site-muted)]">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-white px-2.5 py-1.5">
            <CheckCircle2 size={12} /> Bekleyen cevap varken teslim kapanır
          </span>
        </div>
      </div>
      ...
      {meetRequired ? (
        <section className="rounded-2xl border border-[var(--site-line)] bg-white p-4 sm:p-5">
          ...
          {meetUrl ? (
            ...
            >
              Meet&apos;e gir <ExternalLink size={15} />
            </a>
          ) : (
            <p
              role="alert"
          ...
          <label className="mt-4 flex min-h-[44px] cursor-pointer items-start gap-3 rounded-xl border border-[var(--site-line)] p-3 text-sm text-[var(--site-body)]">
            ...
            />
            <span>
              Meet odasına katıldım ve sınav boyunca görüşmede kalacağımı
              onaylıyorum.
            </span>
          </label>
        </section>
      ...
      <section className="rounded-2xl border border-[var(--site-line)] bg-white p-4 sm:p-5">
        ...
        <button
          type="button"
          onClick={start}
          disabled={busy || (!activeAttempt && (!canStart || !acknowledged))}
          className="panel-primary-button mt-4 w-full sm:w-auto"
        >
          {busy ? (
