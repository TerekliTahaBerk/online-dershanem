...
export function AdminAssignmentPanel({
  ...
  const [mode, setMode] = useState<
    "BULK" | "GROUP" | "CLASS" | "COHORT" | "PACKAGE"
  >("GROUP");
  ...
  const load = useCallback(async () => {
    ...
    if (!response.ok) {
      setMessage({
        text: result.error || "Atamalar yüklenemedi.",
        error: true,
      });
      return;
    }
  ...
  async function assign(body: Record<string, unknown>) {
    ...
    try {
      const response = await fetch(
        `/api/odk/admin/exams/${examId}/assignments`,
        {
          method: "POST",
      ...
      setMessage({
        text: `${result.total} öğrenci atandı (yeni ${result.created}, güncellenen ${result.updated}).`,
        error: false,
      });
      setEmails("");
      await load();
  ...
  async function submit() {
    if (mode === "GROUP" && groupId)
      return assign({ groupId, source: "GROUP" });
    if (mode === "CLASS" && classLevel)
      return assign({ classLevel, source: "CLASS" });
    if (mode === "COHORT" && cohortId)
      return assign({ cohortId, source: "COHORT" });
    if (mode === "PACKAGE" && packageId)
      return assign({ packageId, source: "COHORT" });
    if (mode === "BULK") {
      const tokens = emails
        .split(/[\n,;]+/)
        .map((item) => item.trim())
      ...
      if (!tokens.length)
        return setMessage({
          text: "En az bir e-posta veya kullanıcı ID girin.",
          error: true,
        });
      ...
      {canEdit ? (
        <div className="mt-4 space-y-3">
          <div className="flex flex-wrap gap-2">
            {(["GROUP", "CLASS", "COHORT", "PACKAGE", "BULK"] as const).map(
              (item) => (
                <button
                  key={item}
                  type="button"
              ...
              >
                ...
                {(options?.groups || []).map((group) => (
                  <option key={group.id} value={group.id}>
                    {group.label} · {group.memberCount} öğrenci
                  </option>
                ))}
              </select>
            </label>
          ...
          {mode === "CLASS" ? (
            <label className="panel-field">
              ...
              >
                <option value="">Seçin</option>
                {(options?.classLevels || []).filter(Boolean).map((level) => (
        ...
        <ResponsiveDataTableBody>
          {assignments.slice(0, 50).map((item) => (
            <ResponsiveDataTableRow key={item.id}>
              ...
              <ResponsiveDataTableCell label="Kaynak">
                {item.source}
              </ResponsiveDataTableCell>
              <ResponsiveDataTableCell label="Durum">
