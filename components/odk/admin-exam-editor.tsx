...
type Props = { exam: { id: string; title: string; family: string; status: OdkExamStatus; startsAt: string; endsAt: string; lateEntryMinutes: number; meetRequired: boolean; meetUrl: string; versionStatus: OdkExamVersionStatus; durationMinutes: number; files: { id: string; type: string; fileName: string }[]; questions: Question[] }; outcomes: Outcome[]; issues: Issue[]; resultStats: { attemptCount: number; submittedCount: number; scoredCount: number; examEnded: boolean } };
...
export function AdminExamEditor({ exam, outcomes, issues, resultStats }: Props) {
  ...
  return <div className="space-y-6">
    <section className="sticky top-[76px] z-20 rounded-2xl border border-[var(--site-line)] bg-white/95 p-3 shadow-sm backdrop-blur lg:top-3">
      <div className="flex flex-wrap items-center justify-between gap-3"><nav aria-label="Deneme hazırlama adımları" className="panel-nav-scroll flex gap-2 overflow-x-auto">{["Plan", "PDF", "Cevaplar", "Kontrol", "Sonuç"].map((label, index) => <a key={label} href={`#adim-${index + 1}`} className="min-w-fit rounded-xl bg-[var(--site-bg-warm)] px-3 py-2 text-[11px] font-extrabold text-[var(--site-body)]"><span className="mr-1 text-[var(--brand-olive)]">{index + 1}.</span>{label}</a>)}</nav><div className="flex items-center gap-2"><OdkStatusBadge label={presentation.label} tone={presentation.tone} pulse={exam.status === "LIVE"} />{errors ? <OdkStatusBadge label={`${errors} bloke`} tone="danger" /> : warnings ? <OdkStatusBadge label={`${warnings} uyarı`} tone="warning" /> : <OdkStatusBadge label="Kontroller hazır" tone="success" />}</div></div>
    </section>
    ...
    <section id="adim-5" className="panel-surface scroll-mt-36 p-5 sm:p-6"><h2 className="text-sm font-extrabold">5. Puanlama ve sonuç açıklama</h2><p className="mt-1 text-xs leading-5 text-[var(--site-muted)]">Sonuçlar önce hesaplanır. Öğrenci, veli ve öğretmen ancak ayrıca açıkladığınızda görebilir.</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{[["Katılım", resultStats.attemptCount], ["Teslim", resultStats.submittedCount], ["Puanlandı", resultStats.scoredCount]].map(([label, value]) => <div key={label} className="rounded-2xl bg-[var(--site-bg-warm)] p-4"><p className="text-xl font-black text-[var(--site-ink)]">{value}</p><p className="mt-1 text-xs text-[var(--site-muted)]">{label}</p></div>)}</div><div className="mt-4 flex flex-wrap gap-3">{["SCHEDULED", "LIVE", "ENDED"].includes(exam.status) ? <button type="button" disabled={!resultStats.examEnded || Boolean(busy)} onClick={() => { if (window.confirm("Teslim edilen oturumlar kilitli cevap anahtarıyla puanlansın mı?")) void json(`/api/odk/admin/exams/${exam.id}/score`, "POST", undefined, "Teslimler puanlandı."); }} className="panel-primary-button"><CheckCircle2 size={14} /> Teslimleri puanla</button> : null}{exam.status === "SCORED" ? <button type="button" disabled={resultStats.submittedCount !== resultStats.scoredCount || Boolean(busy)} onClick={() => { if (window.confirm("Sonuçlar ve cevap anahtarı öğrencilere açılacak. Bu işlemden önce puanları kontrol ettiğinizden emin misiniz?")) void json(`/api/odk/admin/exams/${exam.id}/release`, "POST", undefined, "Sonuçlar açıklandı."); }} className="panel-primary-button"><Send size={14} /> Sonuçları açıkla</button> : null}{exam.status === "RELEASED" ? <span className="inline-flex items-center gap-2 rounded-xl bg-[var(--pd-pastel-mint-soft)] px-4 py-2.5 text-xs font-extrabold text-[var(--pd-pastel-mint-ink)]"><CheckCircle2 size={14} /> Sonuçlar açıklandı</span> : null}</div>{!resultStats.examEnded && ["SCHEDULED", "LIVE"].includes(exam.status) ? <p className="mt-3 text-xs font-bold text-amber-700">Puanlama sınavın genel bitiş saatinden sonra açılır.</p> : null}</section>
  </div>;
}
