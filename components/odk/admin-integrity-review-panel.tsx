...
export function AdminIntegrityReviewPanel({
  ...
  async function review(
    action: "MARK_REVIEWED" | "REQUIRE_REVIEW" | "CLEAR_REVIEW",
  ) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span className="panel-metric-icon panel-attention-rose">
          <ShieldAlert size={17} />
        </span>
        <div>
      ...
      {!flagged.length ? (
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          <ul className="space-y-2">
            {flagged.map((item) => {
              const presentation =
                integrityLevelPresentation[item.integrityLevel];
              return (
                <li key={item.id}>
                  <button
                    type="button"
              ...
              <>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      onClick={() => void review("REQUIRE_REVIEW")}
                    >
                      İnceleme gerekli
                    </button>
                    <button
                      ...
                      onClick={() => void review("MARK_REVIEWED")}
                    >
                      {busy ? (
                  ...
                  <ul className="mt-2 space-y-1 font-mono text-[10px]">
                    {detail.events.slice(-40).map((event) => (
                      <li key={event.id}>
                        ...
                        {new Date(event.serverOccurredAt).toLocaleTimeString(
                          "tr-TR",
                        )}
                      </li>
                    ))}
                ...
                {detail.score ? (
                  <p className="mt-3 text-xs font-bold">
                    Skor: {detail.score.correctCount}D /{" "}
                    {detail.score.wrongCount}Y / {detail.score.blankCount}B ·
                    net {Number(detail.score.totalNet).toFixed(2)} ·{" "}
                    {detail.score.publicationStatus}
                  </p>
                ) : null}
              </>
            )}
