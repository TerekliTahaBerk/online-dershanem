...
export function AdminExamCreate({
  ...
  const sectionPreview = useMemo(
    () =>
      template?.sections
        .map((section) => `${section.title} (${section.questionCount})`)
        .join(" · ") || "",
    [template],
  );
  ...
  async function submit(
    ...
    try {
      ...
      if (!response.ok)
        return setMessage({
          text: result.error || "Kayıt oluşturulamadı.",
          error: true,
        });
      if (kind === "exam")
      ...
      setMessage({
        text: "Bağlantı kurulamadı. Kayıt oluşturulmadı; tekrar deneyin.",
        error: true,
      });
    } finally {
      setBusy(null);
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
      <form
        ...
        onSubmit={(event) => void submit(event, "exam")}
      >
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <label className="panel-field">
            ...
            <input
              ...
              required
              minLength={3}
              placeholder="Örn. TYT Denemesi 01"
              onBlur={autoSlug}
            />
          </label>
          <label className="panel-field">
            Kısa adres
            <input name="slug" required placeholder="tyt-denemesi-01" />
          </label>
          <label className="panel-field">
            Sınav türü
            <select
          ...
          {structureMode === "MATH_ONLY" ? (
            <label className="panel-field">
              ...
              <input
                ...
                min={1}
                max={200}
                defaultValue={template?.sections[0]?.questionCount || 40}
              />
            </label>
          ) : null}
          <label className="panel-field">
          ...
          <label className="panel-field sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="Öğrenciye gösterilmeyen kısa iç not"
            />
          </label>
        </div>
        {template ? (
        ...
        <button disabled={busy !== null} className="panel-primary-button mt-5">
          {busy === "exam" ? (
      ...
      <form
        className="panel-surface h-fit p-5 sm:p-6"
        onSubmit={(event) => void submit(event, "series")}
      >
        <div className="flex items-start gap-3">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <label className="panel-field">
            ...
            <input
              ...
              required
              minLength={3}
              placeholder="Örn. TYT Haftalık"
              onBlur={autoSlug}
            />
          </label>
          <label className="panel-field">
            Kısa adres
            <input name="slug" required placeholder="tyt-haftalik" />
          </label>
        ...
        <button
          disabled={busy !== null}
          className="panel-secondary-button mt-5"
        >
          {busy === "series" ? (
            <Loader2 size={14} className="animate-spin" />
          ) : (
