...
import {
  formatOdkDate,
  formatOdkPrice,
} from "@/components/odk/public-package-card";
...
export default async function OdkPackageDetailPage({
  ...
  const description =
    ...
    `${contract.package.title} online deneme paketi`;
    ...
    <div className="site-scope">
      <SchemaJsonLd
        schema={[
          ...
          productJsonLd({
            name: contract.package.title,
            description,
            url: `/odk-paketleri/${slug}`,
            priceCents: contract.package.priceCents,
            originalPriceCents: contract.package.originalPriceCents,
            availability: availability.allowed ? "InStock" : "OutOfStock",
            sku: contract.package.id,
          }),
          faqJsonLd(faq),
        ]}
      />
      ...
      <main id="main-content" tabIndex={-1}>
        <section className="bg-[var(--site-bg-warm)] py-16 sm:py-24">
          <div className="site-container grid gap-10 lg:grid-cols-[1fr_380px]">
            ...
            <aside className="rounded-[28px] border border-[var(--site-line)] bg-white p-6 shadow-xl shadow-slate-900/5">
              ...
              <div className="mt-5">
                <strong className="font-display text-4xl tracking-[-.04em]">
              ...
              {availability.allowed ? (
                ...
                >
                  {item.ctaText || "Güvenli ödemeye geç"}
                </Link>
              ) : (
                <>
                  <button
                    disabled
                    className="site-btn mt-7 w-full cursor-not-allowed justify-center bg-slate-200 text-slate-600"
                  >
        ...
        <section className="site-container py-16 sm:py-20">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {[
              [
                CalendarDays,
                `${contract.exams.length} deneme`,
                "Paket kapsamındaki planlı oturumlar",
              ],
              [
                Clock3,
                access.starts === "PURCHASED_AT"
                  ? "Ödemeyle başlar"
                  : formatOdkDate(access.startsAt || null),
                access.endsAt
                  ? `${formatOdkDate(access.endsAt)} tarihine kadar`
                  : access.durationDays
                    ? `${access.durationDays} gün erişim`
                    : "Sözleşme süresince erişim",
              ],
              [
                FileChartColumn,
                "Ayrıntılı rapor",
                `${reportRights || "Yetkili roller"} erişimi`,
              ],
              [
                ShieldCheck,
                contract.policy.rights.liveService
                  ? "Canlı hizmet"
                  : "Online çözüm",
                "Sınav ve sonuç akışı tek panelde",
              ],
            ].map(([Icon, title, body]) => {
        ...
        <section className="site-container py-16 sm:py-20">
          <div className="grid gap-10 lg:grid-cols-2">
            <div>
              <h2 className="font-display text-4xl tracking-[-.04em]">
                Erişim ve raporlama hakları
              </h2>
              <ul className="mt-7 space-y-4">
                {[
                  [
                    contract.policy.rights.studentReports,
                    "Öğrenci sonuç ve kazanım raporu",
                  ],
                  [contract.policy.rights.parentReports, "Veli rapor erişimi"],
                  [
                    contract.policy.rights.teacherReports,
                    "Öğretmen rapor erişimi",
                  ],
                  [contract.policy.rights.liveService, "Canlı sınav hizmeti"],
                ].map(([enabled, label]) => (
                  <li
                    key={String(label)}
                  ...
                  >
                    ...
                    />
                    {label}:{" "}
                    <strong>{enabled ? "dahil" : "dahil değil"}</strong>
                  </li>
                ))}
              </ul>
            </div>
