...
export default async function OdkStudentResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { exam, score, answerKeyAvailable, weakOutcomeSignals, timeAnalysis, comparison, coachSuggestions, sectionBreakdown } = data;
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK">
    ...
    {Array.isArray(sectionBreakdown) && sectionBreakdown.length ? (
      <PanelCard className="mt-6 p-5"><h2 className="font-extrabold text-dc-ink">Ders bazlı</h2><div className="mt-4 grid gap-2 sm:grid-cols-2">{(sectionBreakdown as Array<{ code?: string; title?: string; net?: number; correct?: number; wrong?: number; blank?: number }>).map((section, index) => <article key={section.code || index} className="rounded-2xl border border-dc-line p-4"><h3 className="text-sm font-bold">{section.title || section.code || `Bölüm ${index + 1}`}</h3><p className="mt-1 text-xs text-dc-ink-muted">Net {section.net == null ? "—" : Number(section.net).toFixed(2)}{section.correct != null ? ` · ${section.correct}D / ${section.wrong ?? 0}Y / ${section.blank ?? 0}B` : ""}</p></article>)}</div></PanelCard>
    ) : null}
    {comparison.length > 1 ? (
      <PanelCard className="mt-6 p-5"><h2 className="font-extrabold text-dc-ink">{exam.family} Net karşılaştırması</h2><div className="mt-4 space-y-2">{comparison.map((item) => <div key={item.examId} className="flex items-center justify-between gap-3 rounded-xl border border-dc-line px-4 py-3 text-sm"><span className={item.examId === id ? "font-extrabold text-dc-brand-strong" : "text-dc-ink"}>{item.title}</span><strong>{item.totalNet.toFixed(2)}</strong></div>)}</div></PanelCard>
    ) : null}
    <section className="mt-6 grid gap-6 xl:grid-cols-2"><PanelCard className="p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-extrabold text-dc-ink">Kazanım görünümü</h2><span className="text-xs font-bold text-dc-ink-muted">{weak.length} gelişim alanı</span></div><div className="mt-4 space-y-3">{score.outcomeScores.map((item) => { const accuracy = Number(item.accuracyRate); const signal = weakOutcomeSignals.find((entry) => entry.outcomeId === item.outcomeId) || null; const avgSec = item.activeDurationMs ? Math.round(item.activeDurationMs / Math.max(1, item.questionCount) / 1000) : null; return <article key={item.outcome.code} className="rounded-2xl border border-dc-line p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-dc-brand-strong">{item.outcome.code}</p><h3 className="mt-1 text-sm font-bold text-dc-ink">{item.outcome.title}</h3><p className="mt-1 text-xs text-dc-ink-muted">{item.outcome.unit.name} · {item.questionCount} soru · {item.correctCount} doğru, {item.wrongCount} yanlış, {item.blankCount} boş{avgSec != null ? ` · ort. ${avgSec} sn/soru` : ""}</p>{signal ? <p className="mt-1 text-[11px] text-dc-ink-muted">{signal.evidenceCount} ölçüm · {signal.questionCount} soru kanıtı{signal.confidence === "LOW" ? " · Az kanıt" : ""}</p> : null}</div><PanelStatusBadge label={`%${accuracy.toFixed(0)}`} tone={accuracy >= 75 ? "success" : accuracy >= 50 ? "warning" : "critical"} /></div><PanelProgress className="mt-3" label={`${item.outcome.code} doğruluk oranı`} value={accuracy} /></article>; })}</div></PanelCard>
    <PanelCard className="p-5"><h2 className="font-extrabold text-dc-ink">Bu sonuçtan sonraki adım</h2><div className="mt-4 space-y-3">{coachSuggestions.map((item) => <PanelAttentionCard key={item.outcomeCode} tone="info" title={`Koçum önerileri · ${item.subject} → ${item.topic}`} body={item.label} action={hasOK ? <TrackedPanelLink href="/panel/ogrenci/plan" className="panel-quick-action panel-quick-action-primary" event={{ name: "odk_recovery_action_started", properties: { product: "ODK", actionKind: "OPEN_PLAN", reasonCode, ageBand, evidenceBand, role: "STUDENT" } }}>Haftalık plana ekle</TrackedPanelLink> : null} />)}{recommendations.map((item, index) => item.tone === "primary" ? <PanelAttentionCard key={`${item.title}-${index}`} tone="info" title={item.title} body={item.detail} action={item.href && item.actionLabel ? <TrackedPanelLink href={item.href} className="panel-quick-action panel-quick-action-primary" event={{ name: "odk_recovery_action_started", properties: { product: "ODK", actionKind: recommendationActionKind(item.href), reasonCode, ageBand, evidenceBand, role: "STUDENT" } }}>{item.actionLabel}</TrackedPanelLink> : null} /> : <PanelCard key={`${item.title}-${index}`} className="border-dc-line-soft p-4"><h3 className="text-sm font-bold text-dc-ink">{item.title}</h3><p className="mt-1 text-xs text-dc-ink-muted">{item.detail}</p>{item.href && item.actionLabel ? <TrackedPanelLink href={item.href} className="panel-quick-action mt-3" event={{ name: "odk_recovery_action_started", properties: { product: "ODK", actionKind: recommendationActionKind(item.href), reasonCode, ageBand, evidenceBand, role: "STUDENT" } }}>{item.actionLabel}</TrackedPanelLink> : null}</PanelCard>)}{!recommendations.length && !coachSuggestions.length ? <PanelEmpty className="mt-0 border-dashed p-5" title="Şu an net bir çalışma önerisi üretilemedi." body="Yeni ölçümle sinyal netleştiğinde bir sonraki adım burada görünür." /> : null}</div>{topSignal ? <DinoExplanationAction deterministicReason={buildOutcomeDeterministicReason(topSignal)} questionKey="student_odk_reason" openLabel="Bu denemeyi açıkla" prepareLabel="Dino ile denemeyi açıkla" /> : null}</PanelCard></section>
    <PanelCard className="mt-6 p-5"><h2 className="font-extrabold text-dc-ink">Zaman analizi</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{timeAnalysis.sections.map((section) => <article key={section.sectionCode} className="rounded-2xl border border-dc-line p-4"><h3 className="text-sm font-bold">{section.sectionTitle}</h3><p className="mt-1 text-xs text-dc-ink-muted">Toplam {Math.round(section.totalActiveMs / 60000)} dk</p><p className="mt-1 text-xs text-dc-ink-muted">Doğru ort: {section.correctAvgMs == null ? "—" : `${Math.round(section.correctAvgMs / 1000)} sn`} · Yanlış ort: {section.wrongAvgMs == null ? "—" : `${Math.round(section.wrongAvgMs / 1000)} sn`}</p></article>)}</div>{timeAnalysis.fastWrongs.length ? <p className="mt-3 text-xs text-dc-ink-muted">Hızlı yanlış: {timeAnalysis.fastWrongs.length} soru · Uzun süreli yanlış: {timeAnalysis.longWrongs.length} soru</p> : null}</PanelCard>
    <PanelCard className="mt-6 p-5"><h2 className="font-extrabold text-dc-ink">Soru cevap dökümü</h2><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{score.questionResults.map((item) => { const ms = data.attempt.timings.find((timing) => timing.questionId === item.questionId)?.activeDurationMs; const outcomes = item.question.outcomes.map((outcome) => outcome.outcome.code).join(", "); return <div key={item.question.questionNumber} className={`rounded-xl p-3 text-xs font-bold ${item.result === "CORRECT" ? "bg-[var(--pd-pastel-mint-soft)] text-[var(--pd-pastel-mint-ink)]" : item.result === "WRONG" ? "bg-[var(--pd-pastel-blush-soft)] text-[var(--pd-pastel-blush-ink)]" : "bg-slate-100 text-slate-700"}`}><p>Soru {item.question.questionNumber}</p><p className="mt-1">Sen: {item.selectedOption || "—"} · Doğru: {item.correctOption}</p>{ms != null ? <p className="mt-1 font-medium opacity-80">{Math.round(ms / 1000)} sn</p> : null}{outcomes ? <p className="mt-1 text-[10px] font-medium opacity-80">{outcomes}</p> : null}</div>; })}</div>{answerKeyAvailable && exam.currentVersion?.files.length ? <a href={`/api/odk/student/exams/${id}/answer-key`} target="_blank" rel="noreferrer" className="panel-quick-action panel-quick-action-primary mt-5 inline-flex"><FileText size={15} /> Cevap anahtarı PDF</a> : null}</PanelCard>
  </PanelShell>;
}
