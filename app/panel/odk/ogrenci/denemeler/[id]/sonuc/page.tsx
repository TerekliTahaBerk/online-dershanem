...
export default async function OdkStudentResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK">
    ...
    <div className="mt-6">
      <PanelPageHeader eyebrow={`${exam.family} matematik sonucu`} title={exam.title} description="Sonucun yalnız kendi cevapların ve denemenin kilitli cevap anahtarı kullanılarak hesaplandı." />
    </div>
