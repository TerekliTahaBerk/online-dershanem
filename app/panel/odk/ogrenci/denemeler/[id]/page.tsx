...
export default async function OdkStudentExamDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK">
    ...
    <section className="mt-6 max-w-2xl">{completed ? <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6"><h2 className="font-extrabold text-emerald-900">Denemen teslim edildi.</h2><p className="mt-2 text-sm leading-6 text-emerald-800">{resultAvailable ? "Sonucun ve kazanım analizin açıklandı." : "Cevapların güvenle kaydedildi. Sonucun sözleşmedeki açıklama zamanında burada görünecek."}</p>{resultAvailable ? <Link href={`/panel/odk/ogrenci/denemeler/${exam.id}/sonuc`} className="mt-4 inline-flex rounded-xl bg-emerald-800 px-4 py-2.5 text-sm font-extrabold text-white">Sonucumu gör</Link> : null}</div> : <StudentExamStart examId={exam.id} meetRequired={exam.meetRequired} meetUrl={exam.meetUrl} canStart={startDecision.ok} startError={startDecision.ok ? null : attemptStartError[startDecision.code]} activeAttempt={activeAttempt} />}</section>
  </PanelShell>;
}
