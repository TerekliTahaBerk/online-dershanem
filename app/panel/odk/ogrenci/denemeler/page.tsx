...
export default async function OdkStudentExamsPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK">
    ...
    <section className="mt-8 space-y-3">{exams.length === 0 ? <div className="rounded-3xl border border-dashed border-[var(--site-line)] bg-white p-8 text-center text-sm text-[var(--site-muted)]">Henüz yayınlanmış bir denemen yok.</div> : exams.map((exam) => { const attempt = exam.attempts[0]; const live = exam.startsAt && exam.endsAt && exam.serverNow >= exam.startsAt && exam.serverNow < exam.endsAt; const complete = attempt && attempt.status !== "IN_PROGRESS"; return <Link key={exam.id} href={`/panel/odk/ogrenci/denemeler/${exam.id}`} className="flex flex-col gap-4 rounded-3xl border border-[var(--site-line)] bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-md sm:flex-row sm:items-center sm:justify-between">
