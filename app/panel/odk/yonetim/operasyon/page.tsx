...
export default async function OdkOperationsPage({
  ...
  const view = views.includes(params.gorunum as (typeof views)[number])
    ? (params.gorunum as (typeof views)[number])
    : "all";
  const now = new Date();
  ...
  const exams = await prisma.odkExam.findMany({
    where: {
      ...
      endsAt: { gte: new Date(now.getTime() - 24 * 60 * 60_000) },
    },
  ...
  const active = attempts.filter(
    (attempt) => attempt.status === "IN_PROGRESS" && attempt.deadlineAt > now,
  );
  const stale = active.filter(
    (attempt) => attempt.lastActivityAt < staleBefore,
  );
  ...
  const displayed = exams.filter((exam) => {
    const isLive = Boolean(
      exam.startsAt && exam.startsAt <= now && exam.endsAt && exam.endsAt > now,
    );
    const hasStale = exam.attempts.some(
      (attempt) =>
        attempt.status === "IN_PROGRESS" &&
        attempt.lastActivityAt < staleBefore,
    );
    const matchesView =
      view === "all" ||
      (view === "live" && isLive) ||
      (view === "attention" && hasStale);
    const matchesQuery =
      !query ||
      exam.title.toLocaleLowerCase("tr-TR").includes(query) ||
      exam.attempts.some((attempt) =>
  ...
  return (
    ...
    >
      <PanelPageHeader
        ...
        title="Sınav akışını tek ekrandan izleyin."
        description="Kalp atışı tarayıcı bağlantısını gösterir; Google Meet katılımının teknik kanıtı değildir. Gecikme durumunda öğrenciyle doğrudan iletişim kurun."
        icon={Activity}
        action={<OdkOperationsRefresh renderedAt={now.toISOString()} />}
      />
      ...
      >
        <label className="panel-field">
          <span className="sr-only">Sınav veya öğrenci ara</span>
          <span className="relative">
            ...
            <input
              ...
              placeholder="Sınav veya öğrenci ara"
              className="panel-input pl-9"
            />
          </span>
        </label>
        <label className="panel-field">
          <span className="sr-only">Operasyon görünümü</span>
          <select name="gorunum" defaultValue={view}>
            <option value="all">Tüm planlı ve canlı</option>
            <option value="live">Yalnız canlı</option>
            <option value="attention">İlgi gerekenler</option>
          </select>
        </label>
        <button className="panel-secondary-button">Uygula</button>
      </form>
      ...
      <section className="mt-5 space-y-4">
        {displayed.map((exam) => {
          ...
          const rows = [...exam.attempts].sort(
            (a, b) =>
              Number(
                b.status === "IN_PROGRESS" && b.lastActivityAt < staleBefore,
              ) -
              Number(
                a.status === "IN_PROGRESS" && a.lastActivityAt < staleBefore,
              ),
          );
          return (
            <PanelCard key={exam.id} className="p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <OdkStatusBadge
                      label={isLive ? "Canlı" : "Planlandı"}
                      tone={isLive ? "danger" : "info"}
                      pulse={isLive}
                    />
                    <span className="text-[10px] font-extrabold text-dc-brand-strong">
                      {exam.family}
                    </span>
                  </div>
                  <h2 className="mt-2 text-lg font-extrabold text-dc-ink">
                    {exam.title}
                  </h2>
                  <p className="mt-1 text-xs text-dc-ink-muted">
                    {exam.startsAt ? time.format(exam.startsAt) : "—"} –{" "}
                    {exam.endsAt ? time.format(exam.endsAt) : "—"}
                  </p>
                </div>
                <Link
              ...
              <div className="mt-4 space-y-2">
                {rows.map((attempt) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <span className="mb-1 block text-[9px] font-bold uppercase text-dc-ink-muted md:hidden">
                          Oturum
                        </span>
                        <OdkStatusBadge
                          label={status.label}
                      ...
                      <div>
                        ...
                        <strong className="text-xs text-dc-ink">
                      ...
                      <div>
                        ...
                        <p className="flex items-center gap-1.5 text-xs text-dc-ink-body">
                          <Video size={13} />
                          {!exam.meetRequired
                            ? "Gerekmiyor"
                            : attempt.meetAcknowledgedAt
                              ? "Katıldığını onayladı"
                              : "Onay eksik"}
                        </p>
                      </div>
                    </article>
                ...
                {!rows.length ? (
                  <p className="rounded-2xl border border-dashed border-dc-line p-6 text-center text-sm text-dc-ink-muted">
                    Henüz sınava giren öğrenci yok.
                  </p>
                ) : null}
              </div>
            </PanelCard>
        ...
        {!displayed.length ? (
          <PanelEmpty
            className="mt-2 border-dashed p-8 text-center"
            title="Bu görünümde izlenecek deneme bulunmuyor."
            body="Filtreleri temizlediğinde uygun sınavlar burada görünür."
          />
        ) : null}
      </section>
    </PanelShell>
