...
export default async function OdkPilotPage() {
  ...
  const readiness = await getOdkPilotReadiness(
    readinessRun?.members || [{ role: "ADMIN" as const }],
    readinessRun?.startedAt,
  );
  const groups = [
    {
    ...
    ...group,
    checks: readiness.checks.filter((check) => check.status === group.status),
  }));
  ...
  return (
    ...
    >
      <PanelPageHeader
        ...
        title="Önce tek koşu, sonra kanıtlı genişleme."
        description="Pilot üyeliği ürün erişiminden ayrıdır. Acil durdurma sınav verisini silmeden erişimi keser; admin kurtarma erişimini korur."
        icon={Rocket}
      />

      <section className="mt-7 grid gap-3 sm:grid-cols-3">
        {groups.map((group) => (
          <article key={group.status} className="panel-metric-card">
            <span className={`panel-metric-icon ${group.className}`}>
              <group.icon size={18} />
            </span>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-[var(--site-muted)]">
              ...
              {readiness.canExpand
                ? "Genişleme kapıları da hazır."
                : "Genel yayın için tüm kapıların hazır olması gerekir."}
            </p>
          </div>
        </div>
      ...
      <div className="mt-5">
        <OdkPilotControl
          ...
          runs={runs.map((run) => ({
            ...
            coverage: roleCoverage(run.members.map((member) => member.role)),
