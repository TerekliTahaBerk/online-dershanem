...
export default async function OdkAdminReportsPage({ searchParams }: { searchParams: Promise<{ ogrenci?: string }> }) {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK" nav={<OdkPanelNav role={session.role} />}><OdkAudienceReports role="ADMIN" basePath="/panel/odk/yonetim/raporlar" students={students} selectedUserId={selected} report={report} /></PanelShell>;
}
