...
export default async function OdkAdminExamsPage({
  ...
  return (
    ...
    >
      <PanelPageHeader
        eyebrow="Denemeler"
        title="Sınav operasyon merkezini yönetin."
      ...
      <section className="mt-9">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <form
            className="grid gap-2 sm:grid-cols-[minmax(180px,1fr)_130px_160px_auto]"
            method="get"
          >
            <label className="panel-field">
              <span className="sr-only">Deneme ara</span>
            ...
            <label className="panel-field">
              <span className="sr-only">Deneme durumu</span>
              <select name="durum" defaultValue={status || ""}>
                <option value="">Tüm durumlar</option>
                {statuses.map((item) => (
                  <option key={item} value={item}>
                    {examStatusPresentation[item].label}
                  </option>
                ))}
              </select>
            </label>
              ...
              {exams.map((exam) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      {exam.currentVersion?.durationMinutes
                        ? `${exam.currentVersion.durationMinutes} dk`
                        : "—"}
                    </td>
                    <td className="px-3 py-3">
                      {exam._count.assignments || started}
                    </td>
