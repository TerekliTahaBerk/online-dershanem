...
export default async function OdkParentReportsPage({ searchParams }: { searchParams: Promise<{ ogrenci?: string }> }) {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} product="ODK"><OdkAudienceReports role="PARENT" basePath="/panel/odk/veli/raporlar" students={students} selectedUserId={selected} report={report} /></PanelShell>;
}
