...
export default async function NotificationsPage({
  ...
  const selectedType = types.some((item) => item.value === params.type)
    ? (params.type as "ALL" | NotificationType)
    : "ALL";
  const selectedStatus = params.status === "unread" ? "unread" : "all";
  const page = Math.max(1, Math.min(1000, Number(params.page) || 1));
  const canChoosePreferences =
  ...
  return (
    ...
    >
      ...
      <div
        className="panel-nav-scroll mt-5 flex gap-2 overflow-x-auto pb-1"
        aria-label="Bildirim türü filtresi"
      >
        {types.map((item) => (
          <Link
            key={item.value}
      ...
      <div
        className={`mt-7 grid gap-5 ${canChoosePreferences ? "xl:grid-cols-[1.2fr_.8fr]" : ""}`}
      >
        <div>
          <NotificationInbox
            key={`${selectedType}-${selectedStatus}-${page}-${unreadTotal}`}
            total={total}
            initialUnread={unreadTotal}
        ...
        {canChoosePreferences ? (
          <NotificationPreferences initial={initial} unread={0} />
        ) : null}
      </div>
    </PanelShell>
  );
