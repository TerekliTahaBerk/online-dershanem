...
function ProductCard({ href, title, description, logo, brand, icon, wideLogo = false }: { href: string; title: string; description: string; logo?: string; brand?: string; icon: React.ReactNode; wideLogo?: boolean }) {
  const ariaLabel = title.endsWith("Paneli") ? `${title}ne git` : `${title} paneline git`;
  return <Link href={href} aria-label={ariaLabel} className="group flex min-h-64 flex-col rounded-[26px] border border-[var(--site-line)] bg-white p-6 shadow-[var(--panel-card-shadow)] transition hover:-translate-y-0.5 hover:border-[var(--brand-olive)]">
