...
export default async function StudentRecoveryPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email}><header><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-[var(--brand-olive)]"><PackageCheck size={15} /> Kaçırdığım ders</p><h1 className="mt-2 text-[26px] font-extrabold leading-[1.25] tracking-[-0.02em]">Biriken iş değil, sıradaki küçük telafi.</h1><p className="mt-2 text-sm leading-6 text-[var(--site-body)]">Özeti incele, kaynakları sırayla aç ve mini kontrolle nerede olduğunu belirt.</p></header><div className="mt-7"><StudentRecoveryPackages rows={rows} /></div></PanelShell>;
}
