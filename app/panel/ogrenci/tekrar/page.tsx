...
export default async function StudentReviewPage() {
  ...
  if (!profile) return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}><PanelEmptyState title="Tekrar profiliniz hazırlanıyor." body="Öğrenci profiliniz tamamlandığında küçük tekrarlar burada açılır." /></PanelShell>;
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}><header><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-[var(--brand-olive)]"><RotateCcw size={15} /> Beş–on dakikalık dönüş</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em]">Bugün yalnız birkaç küçük tekrar.</h1><p className="mt-2 text-sm leading-6 text-[var(--site-body)]">En fazla {dailyReviewLimit} çalışma gösterilir. Yanlış veya emin olmamak ilerlemeni silmez; yalnız sonraki dönüşü yaklaştırır.</p></header><div className="mt-7"><StudentReviewQueue initialItems={items.map((item) => ({ ...item, solutionNote: item.solutionNote || "", dueAt: item.dueAt.toISOString() }))} activeCount={activeCount} masteredCount={masteredCount} /></div></PanelShell>;
}
