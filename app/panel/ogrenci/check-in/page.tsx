...
export default async function StudentCheckInPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}><header><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-[var(--brand-olive)]"><HandHeart size={15} /> Sakin check-in</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em]">Nasıl ilerlediğini fark et, gerekirse yardım iste.</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--site-body)]">Puan, sıralama ve serbest metin yok. Paylaşma kararın sende; veli bu alanı göremez.</p></header><div className="mt-7"><StudentCheckInForm groups={profile.enrollments.map((item) => item.group)} history={history} remaining={Math.max(0, STUDENT_CHECK_IN_WEEKLY_LIMIT - weeklyCount)} /></div></PanelShell>;
}
