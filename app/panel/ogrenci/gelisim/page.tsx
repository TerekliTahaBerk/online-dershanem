...
export default async function StudentProgressPage() {
  ...
  const profile = await prisma.studentProfile.findUnique({ where: { userId: session.userId }, select: { id: true, targetGoal: true } });
  ...
  let streak = 0; for (const item of progress) { if (item.status === "DONE") streak += 1; else break; }
