...
export default async function TeacherMockExamsPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email}><header><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-[var(--brand-olive)]"><ChartNoAxesCombined size={15} /> Denemeden eyleme</p><h1 className="mt-2 text-[26px] font-extrabold leading-[1.25] tracking-[-0.02em]">Neti değil, nedeni görün.</h1><p className="mt-2 text-sm text-[var(--site-body)]">Öğrencinin kendi eğilimini izleyin; hata nedenini düzeltin ve yalnız bir küçük sonraki adımı onaylayın.</p></header><div className="mt-7"><MockExamWorkspace role={session.role} students={students.map((student) => ({ id: student.id, name: student.user.fullName || student.user.email }))} initialExams={exams.map(toMockExamView)} canReview /></div></PanelShell>;
}
