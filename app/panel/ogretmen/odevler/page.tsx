  ...
  return (
    <PanelShell role={session.role} fullName={session.fullName} email={session.email}>
      ...
      <TeacherAssignmentManager
        ...
        assignments={assignments.map((item) => {
          ...
          const summary = summarizeGroupAssignment({
            rows: item.progress.map((row) => ({
              progress: row.status,
              dueAt: item.dueAt,
              submissionStatus: latestByStudent.get(row.studentId) ?? null,
            })),
          });
          return {
            id: item.id,
            ...
            late: summary.late,
            outcomes: item.outcomeLinks.map((link) => link.outcome.code),
        ...
        assignmentEvidenceEnabled={featureFlags.assignmentEvidence}
