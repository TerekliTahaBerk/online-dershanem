...
import { requireTeacherWithScope, teacherCanAccessAttempt } from "@/lib/odk/teacher";
...
const EVENT_LABEL: Record<string, string> = {
  TAB_BLUR: "Sekme bıraktı", TAB_FOCUS: "Sekmeye döndü",
  VISIBILITY_HIDDEN: "Sayfa gizlendi", VISIBILITY_VISIBLE: "Sayfa görünür",
  FULLSCREEN_ENTER: "Tam ekran", FULLSCREEN_EXIT: "Tam ekran çıkışı",
  RIGHT_CLICK: "Sağ tık", COPY: "Kopyalama", PASTE: "Yapıştırma", CUT: "Kesme",
  PRINT: "Yazdırma denemesi", KEY_DEVTOOLS: "Geliştirici araçları",
  ANSWER_CHANGE: "Cevap değişti", NAVIGATE: "Navigasyon", AUTOSAVE: "Otomatik kayıt",
  NETWORK_DROP: "Ağ koptu", NETWORK_RESUME: "Ağ döndü", WARNING_SHOWN: "Uyarı gösterildi",
};
...
export default async function TeacherAttemptDetail({
  params,
}: {
  ...
  const allowed = await teacherCanAccessAttempt(attemptId, userIds);
  if (!allowed) notFound();
  ...
  return (
    <>
      <PageHeader
        ...
        right={
          <Link href={`/panel/ogretmen/odk/ogrencilerim/${attempt.user.id}`} className="od-btn od-btn-ghost">
            Öğrenciye dön
          </Link>
        }
      />
...
function Kpi({ label, value, tone }: { label: string; value: string; tone: "primary" | "ok" | "bad" | "warn" | "neutral" }) {
  ...
  const c = colors[tone];
  return (
    <div style={{ background: c.bg, color: c.color, padding: 16, borderRadius: 12, border: "1px solid var(--pd-line)" }}>
