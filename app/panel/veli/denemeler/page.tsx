...
export default async function ParentMockExamsPage({ searchParams }: { searchParams: Promise<{ studentId?: string }> }) {
  ...
  if (!links.length) return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}><PanelEmptyState title="Öğrenci bağlantınız hazırlanıyor." body="Bağlantı kurulduğunda sakin deneme özeti burada görünür." /></PanelShell>;
