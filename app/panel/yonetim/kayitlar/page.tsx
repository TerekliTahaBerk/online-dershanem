...
export default async function AuditLogsPage({
  ...
  return (
    ...
    >
        ...
        {FILTERS.map((filter) => {
          ...
          return (
            <Link
              ...
              className={`min-w-fit rounded-full border px-3.5 py-2 text-xs font-bold transition ${active ? "border-[var(--brand-olive)] bg-[var(--brand-olive)] text-white" : "border-[var(--site-line)] bg-white text-[var(--site-muted)] hover:text-[var(--site-ink)]"}`}
            >
              {filter.label}
            </Link>
          );
      ...
      <section className="mt-4 overflow-hidden rounded-[14px] border border-[var(--site-line)] bg-white shadow-[var(--panel-card-shadow)]">
        <div className="divide-y divide-[var(--site-line)]">
          {logs.map((log) => (
            <article
              key={log.id}
              className="grid gap-2 px-4 py-4 transition hover:bg-[var(--site-bg-warm)] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-5"
            >
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-[var(--brand-olive-soft)] px-2.5 py-1 text-[10.5px] font-bold text-[var(--brand-olive)]">
                ...
                <p className="mt-1.5 truncate text-xs text-[var(--site-muted)]">
                  {log.actorUserId
              ...
              <time
                ...
                className="text-xs font-medium text-[var(--site-muted)] sm:text-right"
              >
      ...
      <p className="mt-3 text-right text-[11px] text-[var(--site-muted)]">
