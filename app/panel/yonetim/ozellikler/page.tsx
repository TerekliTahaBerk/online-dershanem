...
import {
  getPanelFeatureSnapshot,
  type PanelFeatureStatus,
} from "@/lib/panel-feature-registry";
...
export default async function PanelFeatureInventoryPage() {
  ...
  const actorIds = [
    ...new Set(auditRows.map((r) => r.actorUserId).filter(Boolean) as string[]),
  ];
  ...
  return (
    ...
    >
      ...
      <section
        className={`mt-7 flex items-start gap-3 rounded-[14px] border p-4 ${drift.length ? "border-rose-200 bg-rose-50" : "border-emerald-200 bg-emerald-50"}`}
      >
        {drift.length ? (
          <TriangleAlert className="mt-0.5 shrink-0 text-rose-700" size={18} />
        ) : (
          ...
          <ul className="mt-3 flex flex-col gap-1.5 text-[13.5px] leading-[1.7] text-dc-ink-muted">
            {auditRows.map((row) => (
              <li key={row.id}>
                ...
                {row.actorUserId
                  ? (actorName.get(row.actorUserId) ?? "Bilinmeyen kullanıcı")
                  : "Sistem"}{" "}
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${statusTone[feature.status]}`}
                  >
                ...
                <p className="mt-2 break-all font-mono text-[10.5px] text-[var(--site-muted)]">
                  ...
                  {feature.source === "environment"
                    ? "env ile belirlendi"
                    : `varsayılan ${feature.defaultEnabled ? "açık" : "kapalı"}`}
                </p>
              </div>
