...
export default async function AdminInterventionPage() {
  ...
  const rows = await getInterventionInbox({ role: "ADMIN", userId: session.userId });
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}><AdminPageHeader eyebrow="İnsan müdahalesi" title="Hiçbir sinyal sahipsiz kalmasın." description="Açıklanabilir kural, 24 saat hedefi, sahiplik, sonuç ve yanlış işaret geri bildirimi tek yerde." icon={Inbox} meta={`${rows.length} kayıt`} /><div className="mt-7"><InterventionInbox rows={rows} /></div></PanelShell>;
}
