...
export default async function AdminQualityPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email}>
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-3">
      ...
      <article className="panel-metric-card"><Clock3 size={18} className="text-dc-brand-strong" /><p className="mt-4 text-3xl font-extrabold">{feedbackMedian === null ? "—" : `${feedbackMedian.toLocaleString("tr-TR", { maximumFractionDigits: 1 })} sa`}</p><p className="mt-1 text-xs text-[var(--site-muted)]">Kanıtlı ödev geri bildirim medyanı · {feedbackHours.length} inceleme</p></article>
      <article className="panel-metric-card"><TrendingUp size={18} className="text-dc-ink-muted" /><p className="mt-4 text-3xl font-extrabold">{ready.length}/4</p><p className="mt-1 text-xs text-[var(--site-muted)]">Yayınlanabilir sınav kohortu · eşleşmiş örneklem {cohortSampleBand(pairedTotal)} bandında</p></article>
    </section>
