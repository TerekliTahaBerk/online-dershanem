...
function signed(value: number | null) {
  return value === null
    ? "—"
    : `${value > 0 ? "+" : ""}${value.toLocaleString("tr-TR", { maximumFractionDigits: 1 })} puan`;
}
...
export default async function AdminQualityPage() {
  ...
  const now = new Date();
  const examSince = new Date(now.getTime() - 180 * 86_400_000);
  const qualitySince = new Date(now.getTime() - 30 * 86_400_000);
  const [exams, lessons, assignments, submissions] = await Promise.all([
    prisma.mockExam.findMany({
      where: { takenAt: { gte: examSince } },
      select: {
        studentId: true,
        exam: true,
        takenAt: true,
        sections: {
          select: {
            ...
            correctCount: true,
            incorrectCount: true,
            errors: { select: { category: true } },
          },
        },
      },
    }),
    prisma.lesson.findMany({
      where: { status: "COMPLETED", completedAt: { gte: qualitySince } },
      select: { outcomeLinks: { select: { outcomeId: true } } },
    }),
    prisma.assignment.findMany({
      where: { createdAt: { gte: qualitySince }, isActive: true },
      select: { outcomeLinks: { select: { outcomeId: true } } },
    }),
    prisma.assignmentSubmission.findMany({
      where: { submittedAt: { gte: qualitySince }, reviewedAt: { not: null } },
      select: { submittedAt: true, reviewedAt: true },
    }),
  ]);
  const gains = calculateCohortGains(
    exams.map((exam) => ({
      studentKey: exam.studentId,
      exam: exam.exam,
  ...
  await recordPanelProductEvent(
    {
      name: "cohort_quality_viewed",
      properties: {
        ruleVersion: COHORT_QUALITY_RULE_VERSION,
        readyCohortCount: ready.length,
        suppressedCohortCount: gains.length - ready.length,
        pairedStudentBand: cohortSampleBand(pairedTotal),
      },
    },
    session.role,
  );

  const taggedItems = [...lessons, ...assignments].filter(
    (item) => item.outcomeLinks.length > 0,
  ).length;
  const evidenceCoverage =
    lessons.length + assignments.length
      ? Math.round((taggedItems / (lessons.length + assignments.length)) * 100)
      : null;
  const feedbackHours = submissions.flatMap((item) =>
    item.reviewedAt
      ? [(item.reviewedAt.getTime() - item.submittedAt.getTime()) / 3_600_000]
      : [],
  );
  ...
  for (const exam of exams)
    for (const section of exam.sections)
      for (const error of section.errors) {
        const students = errorStudents.get(error.category) || new Set<string>();
        students.add(exam.studentId);
        errorStudents.set(error.category, students);
      }
  const commonErrors = [...errorStudents.entries()]
    .filter(([, students]) => students.size >= COHORT_MIN_STUDENTS)
  ...
  return (
    ...
    >
      <AdminPageHeader
        eyebrow="Son 180 gün"
        title="Öğrenme kalitesini adil bir zeminde görün."
        description="Aynı öğrencinin başlangıç ve takip ölçümünü eşler; öğrenci ya da öğretmen sıralaması üretmez."
        icon={BarChart3}
        meta={COHORT_QUALITY_RULE_VERSION}
      />
      <section className="mt-6 rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950">
        <div className="flex gap-3">
          <ShieldCheck className="mt-0.5 shrink-0" size={18} />
          <div>
            <h2 className="font-extrabold">Yorumlama sınırı</h2>
            <p className="mt-1 leading-6">
              Bu görünüm nedensel bir “öğretmen etkisi” ölçmez. En az{" "}
              {COHORT_MIN_STUDENTS} öğrenci ve aynı sınav türünde en az{" "}
              {COHORT_MIN_GAP_DAYS} gün aralıklı iki ölçüm olmadan değişim
              istatistikleri gizlenir.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-5 grid gap-3 sm:grid-cols-3">
        ...
        <article className="panel-metric-card">
          <Clock3 size={18} className="text-dc-brand-strong" />
          <p className="mt-4 text-3xl font-extrabold">
            {feedbackMedian === null
              ? "—"
          ...
          {gains.map((gain) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${gain.status === "READY" ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}
                >
                  {gain.status === "READY" ? "Yeterli örnek" : "Gizlendi"}
                </span>
              </div>
              {gain.status === "READY" ? (
                <>
                  ...
                  <dl className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <dd className="font-bold">
                        {percent(gain.positiveChangePercent)}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-[var(--site-muted)]">
                  ...
                  <p className="mt-4 border-t border-[var(--site-line)] pt-3 text-[11px] text-[var(--site-muted)]">
                    ...
                    {gain.dataThrough
                      ? ` · veri ${new Intl.DateTimeFormat("tr-TR").format(gain.dataThrough)} tarihine kadar`
                      : ""}
                  </p>
                </>
              ) : (
