...
export default async function PilotRolloutPage() {
    ...
    prisma.group.findMany({
      ...
      select: {
        ...
        enrollments: {
          where: { endedAt: null },
          select: {
            student: { select: { parents: { select: { parentId: true } } } },
          },
        },
      },
    }),
  ...
  const readinessRoles = cohorts
    .find((cohort) => cohort.status === "ACTIVE")
    ?.members.map((member) => member.role) ||
    cohorts[0]?.members.map((member) => member.role) || ["ADMIN" as const];
  ...
  return (
    ...
    >
      <AdminPageHeader
        eyebrow="Aşama 17 · kontrollü yayın"
        title="Önce küçük kohort, sonra kanıtlı genişleme."
        description="Dört rolü aynı grup bağlamında pilotlayın; güvenlik, restore, bayrak eşliği ve çekirdek SLO kapıları tamamlanmadan erişimi açmayın."
        icon={Rocket}
        meta={
          readiness.canExpand
            ? "Genişlemeye hazır"
      ...
      <div className="mt-5">
        <PilotRolloutControl
          groups={groups.map((group) => ({
            id: group.id,
            name: group.name,
            detail: `${group.teacher.fullName || group.teacher.email} · ${group.enrollments.length} öğrenci · ${new Set(group.enrollments.flatMap((item) => item.student.parents.map((link) => link.parentId))).size} veli`,
          }))}
          cohorts={cohorts.map((cohort) => ({
            id: cohort.id,
            groupName: cohort.groupNameSnapshot,
            status: cohort.status,
            version: cohort.version,
            memberCount: cohort.members.length,
            coverage: roleCoverage(cohort.members.map((member) => member.role)),
