...
export default async function EducationAdminPage() {
  ...
  const [
    ...
    prisma.lesson.findMany({
      where: { startsAt: { gte: new Date(Date.now() - 7 * 86400000) } },
    ...
    featureFlags.learningOutcomes
      ? prisma.learningOutcome.findMany({
          ...
          include: {
            ...
            assignments: { where: { linkedById: session.userId }, take: 1 },
          },
        })
      : Promise.resolve([]),
  ]);
  const name = (item: { fullName: string | null; email: string }) =>
  ...
  for (const lesson of lessons) {
    ...
    if (!current || lesson.startsAt < current)
      nextLessonByGroup.set(lesson.groupId, lesson.startsAt);
  }
  ...
  const openSeats = activeGroups.filter(
    (g) => g.enrollments.length < g.capacity,
  ).length;
  return (
    <PanelShell
      role={session.role}
    ...
    >
      <AdminPageHeader
        eyebrow="Eğitim operasyonu"
        title="Grubu kur, dersi planla."
        description="Tek akıştan öğretmen ekranını, öğrenci özetini ve veli görünümünü birlikte besleyin."
        icon={BookOpenCheck}
      ...
      <section className="mt-7">
        ...
        {activeGroups.length === 0 ? (
          <PanelEmpty
            title="Aktif grup yok."
            body="Grup oluşturulduğunda öğretmeni, doluluğu ve sonraki dersi burada listelenir."
          />
        ) : (
          <div className="mt-3.5">
            <PanelTable
              caption="Aktif gruplar"
              columns={[
                "Grup",
                "Sınav",
                "Ders",
              ...
              {activeGroups.map((group) => {
                ...
                return (
                  <PanelTableRow key={group.id}>
                    ...
                    <PanelTableCell tone={full ? "warn" : undefined}>
                      {filled} / {group.capacity}
                      {full ? " · dolu" : ""}
                    </PanelTableCell>
                    <PanelTableCell>
                      {next ? WHEN.format(next) : "Planlı ders yok"}
                    </PanelTableCell>
                    <PanelTableCell>
                      <Link
                        href={`/panel/yonetim/gruplar/${group.id}`}
        ...
        <TeacherAssignmentManager
          ...
          assignments={assignments.map((assignment) => {
            ...
            return {
              ...
              late: summary.late,
              outcomes: assignment.outcomeLinks.map(
                (link) => link.outcome.code,
              ),
          ...
          outcomes={outcomes.map((outcome) => ({
            ...
            recent: outcome.assignments.length > 0,
          }))}
        />
      </section>
