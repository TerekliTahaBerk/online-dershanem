...
export default async function EducationAdminPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email}>
    ...
    <EducationManagement
      ...
      groups={groupsRaw.map((group) => ({ id: group.id, name: group.name, subject: group.subject, level: group.level || "", teacherId: group.teacher.id, teacherName: name(group.teacher), isActive: group.isActive, capacity: group.capacity, students: group.enrollments.map((item) => ({ id: item.student.id, name: name(item.student.user) })) }))}
