...
export default async function EducationAdminPage() {
  ...
  return <PanelShell role={session.role} fullName={session.fullName} email={session.email} nav={<PanelNav role={session.role} />}>
    ...
    />
    <section className="mt-9"><div className="mb-4"><h2 className="text-lg font-extrabold text-[var(--site-ink)]">Ödev merkezi</h2><p className="mt-1 text-xs text-[var(--site-muted)]">Admin tarafından verilen ödevler öğretmen, öğrenci ve veli ekranlarına aynı anda yansır.</p></div><TeacherAssignmentManager groups={groupsRaw.filter((group) => group.isActive).map((group) => ({ id: group.id, name: group.name, subject: group.subject }))} lessons={lessons.map((lesson) => ({ id: lesson.id, groupId: lesson.groupId, title: lesson.title, startsAt: lesson.startsAt.toISOString() }))} assignments={assignments.map((assignment) => ({ id: assignment.id, groupId: assignment.groupId, groupName: assignment.group.name, title: assignment.title, description: assignment.description || "", dueAt: assignment.dueAt.toISOString(), isActive: assignment.isActive, done: assignment.progress.filter((item) => item.status === "DONE").length, total: assignment.progress.length, outcomes: assignment.outcomeLinks.map((link) => link.outcome.code) }))} learningOutcomesEnabled={featureFlags.learningOutcomes} outcomes={outcomes.map((outcome) => ({ id: outcome.id, code: outcome.code, title: outcome.title, subject: outcome.unit.subject.name, unit: outcome.unit.name, skills: outcome.skills.map((item) => item.skill.name), favorite: outcome.favorites.length > 0, recent: outcome.assignments.length > 0 }))} /></section>
