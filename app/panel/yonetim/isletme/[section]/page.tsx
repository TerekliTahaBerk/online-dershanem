...
import {
  countConversations,
  loadBusinessOverviewKpis,
  loadDeductibleVatCents,
  loadLeadStageCounts,
} from "@/lib/business/queries/overview";
import {
  findDuplicateLeads,
  loadFunnelLeads,
  loadLeadDetail,
  loadLeadWorklist,
  loadSalesOwners,
} from "@/lib/business/queries/leads";
...
const permissionFor = (section: BusinessSectionSlug): BusinessPermission =>
...
export default async function BusinessSectionPage({
  ...
  if (
    !businessFlags.panel ||
    (FINANCE_SECTIONS.includes(section) && !businessFlags.finance)
  )
    notFound();
  ...
  const allowedSections = visibleSections(granted, {
    financeEnabled: businessFlags.finance,
  });
  ...
  const scopedUnits = access.units.filter(
    (unit) => product === "ALL" || unit.product === product,
  );
  const unitIds = (scopedUnits.length ? scopedUnits : access.units).map(
    (unit) => unit.id,
  );
  const selectedConversation =
    typeof query.conversation === "string" ? query.conversation : undefined;
  const selectedLeadId =
    typeof query.lead === "string" ? query.lead : undefined;
  const messageCursor =
    typeof query.messageCursor === "string" ? query.messageCursor : undefined;
  ...
  const leadFocus: LeadListFocus =
    leadFocusRaw === "overdue" ||
    leadFocusRaw === "no_activity" ||
    leadFocusRaw === "all" ||
    leadFocusRaw === "mine"
      ? leadFocusRaw
      : "today";
  const leadStageFilter =
    ...
    Object.values(LeadStage).includes(query.stage as LeadStage)
      ? (query.stage as LeadStage)
      : "ALL";
  const leadSourceFilter =
    typeof query.source === "string" &&
    Object.values(LeadSource).includes(query.source as LeadSource)
      ? (query.source as LeadSource)
      : "ALL";
  const leadInterestFilter =
    typeof query.interest === "string" &&
    Object.values(ProductInterest).includes(query.interest as ProductInterest)
      ? (query.interest as ProductInterest)
      : "ALL";
  const leadOwnerFilter = typeof query.owner === "string" ? query.owner : "ALL";
  const leadCampaignFilter =
    typeof query.campaign === "string" ? query.campaign : "ALL";
  const leadQuery = typeof query.q === "string" ? query.q : undefined;
  const inboxWhere: Prisma.BusinessConversationWhereInput = {
    ...
    Object.values(ConversationStatus).includes(
      ...
      : {}),
    ...(query.unread === "1" ? { unreadCount: { gt: 0 } } : {}),
    ...(typeof query.temperature === "string" &&
    Object.values(LeadTemperature).includes(
      query.temperature as LeadTemperature,
    )
      ? { temperature: query.temperature as LeadTemperature }
      : {}),
    ...(typeof query.ai === "string" &&
    Object.values(AiMode).includes(query.ai as AiMode)
      ? { aiMode: query.ai as AiMode }
      : {}),
  ...
  const [
    ...
    selectedConversation
      ...
      : Promise.resolve([]),
    needsInbox
      ? prisma.businessConversation.findMany({
          where: inboxWhere,
          include: {
            lead: true,
          ...
          take: 31,
          ...(conversationCursor
            ? { cursor: { id: conversationCursor }, skip: 1 }
            : {}),
        })
      : Promise.resolve([]),
    selectedConversation
      ? prisma.businessConversation.findFirst({
          where: { id: selectedConversation, businessUnitId: { in: unitIds } },
          include: {
            lead: {
              include: {
            ...
            aiExecutions: { orderBy: { createdAt: "desc" }, take: 1 },
          },
        })
      : Promise.resolve(null),
    needsCampaigns
      ? prisma.businessCampaign.findMany({
          where: { businessUnitId: { in: unitIds } },
          include: { adSets: { include: { advertisements: true } } },
          orderBy: { createdAt: "desc" },
          take: 50,
        })
      : Promise.resolve([]),
    needsCampaigns
      ? prisma.businessAdvertisement.findMany({
          where: { adSet: { campaign: { businessUnitId: { in: unitIds } } } },
    ...
    section === "sistem-kayitlari"
      ? prisma.backgroundJob.findMany({
          where: {
            OR: [{ businessUnitId: { in: unitIds } }, { businessUnitId: null }],
          },
          orderBy: { createdAt: "desc" },
          take: 50,
        })
    ...
    section === "sistem-kayitlari"
      ...
      : Promise.resolve([]),
    needsKpis
      ? loadBusinessOverviewKpis({ unitIds, from, to })
      : Promise.resolve(null),
    ...
    section === "ayarlar"
      ...
      : Promise.resolve([]),
    section === "vergiler"
      ? loadDeductibleVatCents({ unitIds, from, to })
      : Promise.resolve(0),
    section === "ayarlar"
    ...
    section === "ayarlar"
      ...
      : Promise.resolve([]),
    section === "mesaj-kutusu"
      ? countConversations(inboxWhere)
      : Promise.resolve(0),
    section === "satis-hunisi"
      ? loadLeadStageCounts(unitIds)
    ...
    section === "adaylar" || section === "genel-bakis" || section === "raporlar"
      ? loadSalesOwners(unitIds)
      : Promise.resolve([]),
    section === "genel-bakis" || section === "raporlar"
      ? loadLeadAnalytics({ unitIds, from, to })
      : Promise.resolve(null),
  ]);
  const conversations = conversationsPage.slice(0, 30);
  const nextCursor =
    conversationsPage.length > 30 ? conversationsPage[30].id : null;
  const income = kpis?.incomeCents ?? 0;
  const expense = kpis?.expenseCents ?? 0;
  const selected = selectedDetail;
  const messages = messagesPage.slice(0, 100);
  const nextMessageCursor =
    messagesPage.length > 100 ? messagesPage[100].id : null;
  const orderedMessages = [...messages].reverse();
  const leadDetail =
    section === "adaylar" && selectedLeadId
      ? await loadLeadDetail(selectedLeadId, unitIds)
      : null;
  const leadLifecycleDetail =
    section === "adaylar" && selectedLeadId
      ? await loadLeadLifecycleDetail(selectedLeadId, {
          businessUnitIds: unitIds,
        })
      : null;
  const leadDuplicates = leadDetail
    ...
    : [];
  const ownerOptions = (salesOwners.length ? salesOwners : staff).map(
    (item) => ({
      id: item.user.id,
  ...
  const uniqueOwners = Array.from(
    new Map(ownerOptions.map((o) => [o.id, o])).values(),
  );
  const ownerNameMap: Record<string, string> = Object.fromEntries(
    uniqueOwners.map((o) => [o.id, o.fullName]),
  );
  const provisioning = leadDetail
    ? {
  ...
  return (
    <PanelShell
      ...
      fullName={access.session.fullName}
      email={access.session.email}
      workspace="BUSINESS"
      nav={<BusinessNav active={section} allowed={allowedSections} />}
    ...
    >
      ...
      <div className="mt-5">
        {section === "genel-bakis" && kpis && (
          <>
            ...
            >
              <label className="text-xs">
                ...
                <input
                  name="from"
                  type="date"
                  defaultValue={formatIstanbulDateInput(from)}
              ...
              <label className="text-xs">
                ...
                <input
                  name="to"
                  type="date"
                  defaultValue={formatIstanbulDateInput(to)}
                  className="ml-2 rounded-xl border p-2"
                />
              </label>
              <select
            ...
            {rangeNotice ? (
              <p
                role="status"
                className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-950"
              >
                {rangeNotice}
              </p>
            ) : null}
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
              <Card
                label="Konuşma"
                value={kpis.conversationTotal}
                icon={Inbox}
              />
              <Card label="Yeni aday" value={kpis.newLeads} icon={UsersRound} />
              <Card label="Satış" value={kpis.wonLeads} icon={BarChart3} />
              <Card
                label="Reklam harcaması"
                value={tl.format(kpis.adSpendCents / 100)}
                icon={Megaphone}
              />
              <Card
                label="Net gelir"
                value={tl.format(kpis.netCents / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="İnsan bekleyen"
                value={kpis.waitingHumanConversations}
                icon={Bot}
              />
            </section>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <p className="rounded-xl bg-emerald-50 p-4 text-sm">
                  ...
                  <strong className="block text-lg">
                    {tl.format(income / 100)}
                  </strong>
                </p>
                <p className="rounded-xl bg-rose-50 p-4 text-sm">
                  Gider{" "}
                  <strong className="block text-lg">
                    {tl.format(expense / 100)}
                  </strong>
                </p>
                <p className="rounded-xl bg-amber-50 p-4 text-sm">
                  Tahmini net{" "}
                  <strong className="block text-lg">
                    {tl.format(kpis.netCents / 100)}
                  </strong>
                </p>
              </div>
              <div
                className="mt-5 grid gap-3 md:grid-cols-3"
                aria-label="Satış hunisi grafiği"
              >
          ...
          <>
            ...
            >
              ...
              >
                ...
                {Object.values(ConversationStatus).map((value) => (
              ...
              <select
                name="ai"
                defaultValue={typeof query.ai === "string" ? query.ai : "ALL"}
                aria-label="AI modu"
                className="rounded-xl border px-3 py-2 text-xs"
              >
                <option value="ALL">Tüm AI modları</option>
                {Object.values(AiMode).map((value) => (
                  <option key={value}>{value}</option>
                ))}
              </select>
              <label className="flex items-center gap-2 rounded-xl border px-3 text-xs">
                <input
                  type="checkbox"
                  name="unread"
                  value="1"
                  defaultChecked={query.unread === "1"}
                />{" "}
                Yalnız okunmamış
              </label>
            ...
            <div className="grid gap-4 xl:grid-cols-[360px_1fr]">
              <section className="panel-surface overflow-hidden">
                ...
                <div className="divide-y divide-[var(--site-line)]">
                  {conversations.length ? (
                    ...
                    <Empty text="Filtreyle eşleşen konuşma yok." />
                  )}
                </div>
                {nextCursor ? (
                  <Link
                    href={`?cursor=${nextCursor}`}
                    className="m-3 block rounded-xl border p-2 text-center text-xs font-bold"
                  >
                    Sonraki sayfa
                  </Link>
                ) : null}
              ...
              <section className="panel-surface p-5">
                {selected ? (
                  <>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <form action={markConversationRead}>
                        <input type="hidden" name="id" value={selected.id} />
                        <button className="rounded-xl border px-3 py-2 text-xs">
                          Okundu işaretle
                        </button>
                      </form>
                      <form action={requestAISuggestion}>
                        <input type="hidden" name="id" value={selected.id} />
                        <button className="rounded-xl border px-3 py-2 text-xs font-bold">
                          AI önerisi üret
                        </button>
                      </form>
                      <form action={assignConversation} className="flex">
                        <input type="hidden" name="id" value={selected.id} />
                        <select
                          aria-label="Sorumlu personel"
                          name="assignedUserId"
                          defaultValue={selected.assignedUserId || ""}
                          className="rounded-l-xl border px-2 text-xs"
                        >
                        ...
                        <button className="rounded-r-xl border px-2 text-xs">
                          Ata
                        </button>
                      </form>
                      <form action={addConversationTag} className="flex">
                        <input type="hidden" name="id" value={selected.id} />
                        <input
                          aria-label="Etiket"
                          name="tag"
                          required
                          placeholder="Etiket"
                          className="w-24 rounded-l-xl border px-2 text-xs"
                        />
                        <button className="rounded-r-xl border px-2 text-xs">
                          Ekle
                        </button>
                    ...
                    {nextMessageCursor ? (
                      <Link
                        href={`?conversation=${selected.id}&messageCursor=${nextMessageCursor}`}
                        className="mt-4 block text-center text-xs font-bold underline"
                      >
                        Daha eski 100 mesajı yükle
                      </Link>
                    ) : null}
                    <div
                      className="mt-5 max-h-[520px] space-y-3 overflow-auto"
                      aria-live="polite"
                    >
                      {orderedMessages.map((m) => (
                        ...
                        >
                          ...
                          <p className="mt-1 text-[9px] opacity-70">
                            {m.senderType} · {m.status} ·{" "}
                            {dt.format(m.occurredAt)}
                            {m.failureCode ? ` · ${m.failureCode}` : ""}
                          </p>
                        </article>
                      ))}
                    </div>
                    {selected.aiExecutions[0]?.decision &&
                    typeof selected.aiExecutions[0].decision === "object" &&
                    "reply" in selected.aiExecutions[0].decision &&
                    typeof selected.aiExecutions[0].decision.reply ===
                      "string" ? (
                      <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-3">
                        <p className="text-[10px] font-extrabold uppercase text-sky-800">
                          AI cevap önerisi · düzenleyip gönderilebilir
                        </p>
                        <ReplyForm
                          conversationId={selected.id}
                          initialText={selected.aiExecutions[0].decision.reply}
                        />
                      </section>
                    ) : null}
                    ...
                    {selected.lead ? (
                      <div className="mt-5 grid gap-3 lg:grid-cols-2">
                        <section className="rounded-2xl border p-3">
                          ...
                          <form action={addLeadNote} className="mt-2 flex">
                            <input
                              type="hidden"
                              name="leadId"
                          ...
                          <form
                            ...
                            className="mt-2 grid grid-cols-[1fr_auto_auto]"
                          >
                            <input
                              type="hidden"
                              name="leadId"
                          ...
                          {selected.lead.activities.map((item) => (
                            <p
                              key={item.id}
                              className="mt-2 border-t pt-2 text-[10px]"
                            >
                              {item.type} · {dt.format(item.createdAt)}
                            </p>
                          ))}
                          {selected.lead.tasks.map((item) => (
                            <p key={item.id} className="mt-2 text-[10px]">
                              Görev: {item.title} ·{" "}
                              {item.completedAt ? "Tamamlandı" : "Açık"}
                            </p>
                          ))}
                          {selected.lead.financialTransactions.map((item) => (
                            <p key={item.id} className="mt-2 text-[10px]">
                              Ödeme: {item.description} ·{" "}
                              {tl.format(item.netCents / 100)} · {item.status}
                            </p>
                          ))}
                        </section>
                        <section className="rounded-2xl border p-3">
                          ...
                          {selected.lead.attributions.map((item) => (
                            <p key={item.id} className="mt-2 text-[10px]">
                              {item.model} ·{" "}
                              {item.campaign?.name ||
                  ...
                  <Empty text="Detay için bir konuşma seçin." />
                )}
              </section>
            </div>
          </>
                ...
                <div className="mb-6 space-y-4">
                  ...
                  {leadLifecycleDetail ? (
                    <LeadLifecyclePanel
                      ...
                      linkAction={
                        can("lead:write") ? linkLeadLifecycle : undefined
                      }
                    />
                  ) : null}
                </div>
        ...
        {(section === "kampanyalar" || section === "reklamlar") && (
          <>
            {section === "kampanyalar" ? (
              <form
              ...
              >
                ...
                <input
                  required
                  name="adSetName"
                  aria-label="Reklam seti"
                  placeholder="Reklam seti"
                  className="rounded-xl border p-2 text-xs"
                />
                <input
                  required
                  name="name"
                  aria-label="Reklam adı"
              ...
              rows={ads.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-t">
                    ...
                    <td className="p-3">{a.adSet.campaign.name}</td>
                    <td className="p-3">{tl.format(a.spentCents / 100)}</td>
                    <td className="p-3">{tl.format(metrics.cpmCents / 100)}</td>
                    <td className="p-3">{tl.format(metrics.cpcCents / 100)}</td>
                    <td className="p-3">%{(metrics.ctr * 100).toFixed(2)}</td>
                    <td className="p-3">
                      {tl.format(metrics.costPerMessageCents / 100)}
                    </td>
                    <td className="p-3">
                      {tl.format(metrics.costPerLeadCents / 100)}
                    </td>
                    <td className="p-3">
                      {tl.format(metrics.costPerSaleCents / 100)}
                    </td>
                    <td className="p-3">{metrics.roas.toFixed(2)}x</td>
                  </tr>
                );
              })}
            />
        ...
        {(section === "gelirler" || section === "giderler") && (
          <>
            {can("finance:write") ? (
              <form
              ...
              >
                ...
                <select
                  ...
                  name="kind"
                  defaultValue={
                    section === "giderler" ? "EXPENSE" : "MANUAL_INCOME"
                  }
                  className="rounded-xl border px-3 py-2 text-sm"
                >
                  <option>MANUAL_INCOME</option>
            ...
            <TransactionTable
              rows={transactions.filter((x) =>
                section === "giderler"
                  ? x.kind === "EXPENSE"
                  : x.kind !== "EXPENSE",
              )}
              canReverse={can("finance:reverse")}
            />
          </>
        )}
        {section === "vergiler" && kpis && (
          <>
            <div
              role="note"
              className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"
            >
            ...
            <section className="grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
              <Card
                label="Vergi dahil gelir"
                value={tl.format(income / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="Hesaplanan KDV"
                value={tl.format(kpis.vatCents / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="İndirilecek KDV"
                value={tl.format(deductibleVatCents / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="Stopaj"
                value={tl.format(kpis.withholdingCents / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="Komisyon"
                value={tl.format(kpis.commissionCents / 100)}
                icon={CircleDollarSign}
              />
              <Card
                label="Tahmini net kâr"
                value={tl.format((income - expense - kpis.vatCents) / 100)}
                icon={BarChart3}
              />
            </section>
              ...
              <div>
                ...
                {periods.map((item) => (
                  <p key={item.id} className="mt-2 text-xs">
                    {dt.format(item.startsAt)} – {dt.format(item.endsAt)} ·{" "}
                    <strong>{item.status}</strong>
                  </p>
                ))}
              </div>
              <form action={lockAccountingPeriod} className="grid gap-2">
                <UnitField units={access.units} />
                <h2 className="text-sm font-extrabold">Dönem kilitle</h2>
                <input
        ...
        {section === "mutabakat" && (
          <>
            ...
            {reconciliations.length ? (
              <DataTable
                ...
                rows={reconciliations.map((x) => (
                  <tr key={x.id} className="border-t">
                    ...
                    <td className="p-3">{x.externalId || "—"}</td>
                    <td className="p-3">
                      {tl.format((x.expectedCents || 0) / 100)}
                    </td>
                    <td className="p-3">
                      {tl.format((x.actualCents || 0) / 100)}
                    </td>
                    ...
                    <td className="p-3">
                      {x.status !== "MATCHED" ? (
                        ...
                        >
                          ...
                          <button className="underline">Kaydet</button>
                        </form>
                      ) : (
                        "—"
                      )}
              ...
              <Empty text="Mutabakat kaydı yok; taramayı çalıştırın." />
            )}
          </>
        )}
        {section === "raporlar" && (
          ...
          <>
            <div className="grid gap-4 xl:grid-cols-2">
              ...
              >
                ...
                <input
                  aria-label="Prompt adı"
                  name="name"
                  required
                  defaultValue="instagram-sales"
                  className="rounded-xl border p-2 text-sm"
                />
                <textarea
                  aria-label="Ek sistem kuralları"
                  name="systemPrompt"
                  required
                  minLength={20}
                  placeholder="Marka dili ve izinli ek sistem kuralları"
                  className="min-h-32 rounded-xl border p-3 text-sm"
                />
                <button className="rounded-xl border p-2 text-sm font-bold">
                  Yeni aktif sürüm oluştur
                </button>
                {prompts.map((item) => (
            ...
            {knowledge.length ? (
                  ...
                  <article key={x.id} className="panel-surface p-4">
                    <div className="flex justify-between">
                      ...
                      <span className="text-[10px]">
                        v{x.version} · {x.isActive ? "Aktif" : "Pasif"}
                      </span>
                    </div>
                    <form
                      action={versionKnowledgeEntry}
                      className="mt-2 grid gap-2"
                    >
                      <input type="hidden" name="id" value={x.id} />
                      <textarea
                        aria-label={`${x.title} içeriği`}
                        name="content"
                        defaultValue={x.content}
                        className="min-h-24 rounded-xl border p-2 text-sm"
                      />
                      <select
                        aria-label="Aktiflik"
                        name="isActive"
                        defaultValue={String(x.isActive)}
                        className="rounded-xl border p-2 text-xs"
                      >
              ...
              <Empty text="Bilgi merkezi kaydı yok." />
            )}
          </>
        )}
        {section === "otomasyon-kurallari" && (
        ...
        {section === "entegrasyonlar" && (
          <>
            <div className="mb-4 flex flex-wrap gap-2">
              <Link
                href="/api/integrations/instagram/health"
                className="rounded-xl border px-4 py-2 text-xs font-bold"
              >
                Instagram canlı sağlık kontrolü
              </Link>
              <form action={syncMetaAds}>
                <button className="rounded-xl border px-4 py-2 text-xs font-bold">
                  Meta Ads senkronize et
                </button>
              </form>
            </div>
        ...
        {section === "sistem-kayitlari" && (
          <div className="grid gap-4 xl:grid-cols-2">
            <section className="panel-surface p-4">
              ...
              {jobs.map((x) => (
                <p key={x.id} className="border-t py-2 text-xs">
                  <strong>{x.type}</strong> · {x.status} · deneme {x.attempts}/
                  {x.maxAttempts}
                </p>
              ))}
            </section>
            <section className="panel-surface p-4">
              ...
              {audit.map((x) => (
                <p key={x.id} className="border-t py-2 text-xs">
                  <strong>{x.action}</strong> · {x.entityType} ·{" "}
                  {dt.format(x.createdAt)}
                </p>
              ))}
            </section>
          </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <section className="panel-surface p-5">
              ...
              <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-[var(--site-body)]">
                ...
                <li>
                  Finans kayıtları hard delete edilmez; ters kayıt kullanılır.
                </li>
                <li>Entegrasyon anahtarları istemciye gönderilmez.</li>
              </ul>
              {can("settings:write") ? (
                <form
                ...
                >
                  <label className="text-xs">
                    KVKK saklama süresi (gün)
                    <input
                      name="retentionDays"
                      type="number"
                      min="30"
                      max="3650"
                      defaultValue={settingsUnits[0]?.retentionDays ?? 730}
                      className="ml-2 rounded-xl border p-2"
                    />
                  </label>
                  <button className="rounded-xl border px-4 py-2 text-xs font-bold">
                    Tüm yetkili iş birimlerine uygula
                  </button>
                </form>
              ) : null}
            </section>
              ...
              <section className="panel-surface p-5">
                ...
                {can("role:write") ? (
                  <form action={assignBusinessRole} className="mt-3 grid gap-2">
                    ...
                    <select
                      required
                      aria-label="İş birimi"
                      name="businessUnitId"
                      className="rounded-xl border p-2 text-xs"
                    >
                      {settingsUnits.map((unit) => (
                        <option key={unit.id} value={unit.id}>
                          {unit.name}
                        </option>
                      ))}
                ...
                {roleAssignments.map((item) => (
                  <div
                    key={item.id}
                    className="mt-2 flex items-center justify-between gap-2 border-t pt-2 text-[10px]"
                  >
                    <span>
                      {item.user.fullName} · {item.businessUnit.name} ·{" "}
                      {item.role}
                    </span>
                    {can("role:write") ? (
                      <form action={revokeBusinessRole}>
                        <input type="hidden" name="id" value={item.id} />
                        <button className="font-bold text-rose-700 underline">
                          Kaldır
                        </button>
                      </form>
                    ) : null}
...
function TransactionTable({
  ...
  rows: Awaited<ReturnType<typeof prisma.financialTransaction.findMany>>;
  canReverse: boolean;
}) {
  return (
    <DataTable
      ...
      rows={rows.map((x) => (
        <tr key={x.id} className="border-t">
          <td className="p-3">{dt.format(x.transactionAt)}</td>
          <td className="p-3 font-bold">{x.description}</td>
          <td className="p-3">{x.source}</td>
          <td className="p-3">{x.category}</td>
          <td className="p-3">{tl.format(x.netCents / 100)}</td>
          <td className="p-3">{tl.format(x.vatCents / 100)}</td>
          <td className="p-3">{x.status}</td>
          <td className="p-3">
            {!x.cancelledAt && canReverse ? (
              <form action={reverseFinancialTransaction}>
                <input type="hidden" name="id" value={x.id} />
                <button className="text-rose-700 underline">Ters kayıt</button>
              </form>
            ) : x.cancelledAt ? (
              "İptal"
            ) : (
