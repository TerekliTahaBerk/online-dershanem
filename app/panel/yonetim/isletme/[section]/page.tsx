...
import { addConversationTag, addLeadNote, assignBusinessRole, assignConversation, createAdvertisement, createAutomationRule, createCampaign, createFinancialTransaction, createKnowledgeEntry, createLeadTask, createManualLead, createPromptVersion, linkLeadLifecycle, lockAccountingPeriod, markConversationRead, requestAISuggestion, resolveReconciliation, reverseFinancialTransaction, revokeBusinessRole, runReconciliation, setConversationControl, setManualAttribution, syncMetaAds, updateRetentionSettings, versionKnowledgeEntry } from "../actions";
...
export default async function BusinessSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [messagesPage, conversationsPage, selectedDetail, _overviewLeads, campaigns, ads, transactions, reconciliations, knowledge, rules, integrations, jobs, audit, kpis, staff, periods, prompts, settingsUnits, deductibleVatCents, settingsUsers, roleAssignments, inboxTotal, funnelCounts, leadWorklist, funnelLeads, salesOwners, leadAnalytics] = await Promise.all([
    ...
    section === "otomasyon-kurallari" ? prisma.automationRule.findMany({ where: { businessUnitId: { in: unitIds } }, include: { executions: { orderBy: { createdAt: "desc" }, take: 3 } }, orderBy: { updatedAt: "desc" }, take: 100 }) : Promise.resolve([]),
  ...
  return <PanelShell role={access.session.role} fullName={access.session.fullName} email={access.session.email} workspace="BUSINESS" nav={<BusinessNav active={section} allowed={allowedSections} />}>
    ...
    <div className="mt-5">
      ...
      {section === "otomasyon-kurallari" && <><form action={createAutomationRule} className="panel-surface mb-4 grid gap-3 p-4 sm:grid-cols-3 xl:grid-cols-6"><UnitField units={access.units}/><input aria-label="Kural adı" name="name" required placeholder="Kural adı" className="rounded-xl border px-3 py-2 text-sm"/><select aria-label="Tetikleyici" name="triggerType" className="rounded-xl border px-3 py-2 text-sm"><option>NEW_MESSAGE</option><option>HOT_LEAD</option><option>PAYMENT_COMPLETED</option><option>COMPLAINT</option><option>UNANSWERED_HOT_LEAD</option></select><select aria-label="Koşul sıcaklığı" name="temperature" className="rounded-xl border px-3 py-2 text-sm"><option value="">Her sıcaklık</option><option>COLD</option><option>WARM</option><option>HOT</option></select><select aria-label="Aksiyon" name="actionType" className="rounded-xl border px-3 py-2 text-sm"><option>SUGGEST_AI_REPLY</option><option>ASSIGN_SALES</option><option>MARK_WON</option><option>NOTIFY_ADMIN</option><option>ADD_TAG</option><option>STOP_AI</option><option>MARK_SPAM</option><option>CREATE_TASK</option></select><input aria-label="Etiket" name="tag" placeholder="ADD_TAG etiketi" className="rounded-xl border px-3 py-2 text-sm"/><button className="rounded-xl bg-[var(--brand-olive)] p-2 text-sm font-bold text-white">Kural ekle</button></form>{rules.length?<div className="space-y-3">{rules.map(x=><article key={x.id} className="panel-surface p-4"><div className="flex justify-between"><div><strong>{x.name}</strong><p className="text-xs text-[var(--site-muted)]">{x.triggerType}</p></div><span className="text-xs font-bold">{x.isActive?"Aktif":"Pasif"}</span></div>{x.executions.map((execution)=><p key={execution.id} className="mt-2 border-t pt-2 text-[10px]">{execution.result} · {execution.durationMs} ms · {dt.format(execution.createdAt)}{execution.errorCode?` · ${execution.errorCode}`:""}</p>)}</article>)}</div>:<Empty text="Otomasyon kuralı yok."/>}</>}
