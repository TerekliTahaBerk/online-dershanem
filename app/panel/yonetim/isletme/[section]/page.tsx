...
import { FINANCE_SECTIONS, SECTION_LABELS, SECTION_PERMISSIONS, businessMobilePrimaryNav, isBusinessSection, visibleSections, type BusinessSectionSlug } from "@/lib/business/sections";
...
export default async function BusinessSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <PanelShell role={access.session.role} fullName={access.session.fullName} email={access.session.email} workspace="BUSINESS" nav={<BusinessNav active={section} allowed={allowedSections} />} mobileQuickItems={businessMobilePrimaryNav(allowedSections)}>
