          ...
          <div className="site-container grid items-center gap-12 lg:grid-cols-[1.12fr_.88fr]">
            <div>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                ...
                >
                  Paketleri incele <ArrowRight size={18} />
                </a>
                <Link
                  href="/iletisim"
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
              {[
                [
                  TimerReset,
                  "Planlı oturum",
                  "Deneme günü, başlangıç ve bitiş saati paket sözleşmesinde görünür.",
                ],
                [
                  BarChart3,
                  "Kazanım raporu",
                  "Doğru, yanlış ve boşların ötesinde konu bazlı gelişimi izleyin.",
                ],
                [
                  ShieldCheck,
                  "Kontrollü erişim",
                  "Ödeme yalnız satış ve operasyon koşulları eksiksiz olduğunda açılır.",
                ],
              ].map(([Icon, title, body]) => {
        ...
        >
          ...
          {packages.length ? (
            <div className="mt-10 grid gap-6 lg:grid-cols-2">
              {packages.map((item) => (
                <PublicOdkPackageCard
                  key={item.contract.package.id}
                  item={item}
                />
              ))}
            </div>
          ) : (
            <div className="mt-10 rounded-[28px] border border-amber-200 bg-amber-50 p-8">
              <h3 className="font-display text-2xl text-amber-950">
                Yeni dönem paketleri hazırlanıyor.
              </h3>
              <p className="mt-3 max-w-2xl leading-7 text-amber-900">
                Eksik veya satışa hazır olmayan bir paket için ödeme yolu
                göstermiyoruz. Duyuru almak için bizimle iletişime
                geçebilirsiniz.
              </p>
              <Link
                href="/iletisim"
                className="site-btn site-btn-secondary mt-6"
              >
                Bilgi alın
              </Link>
            </div>
