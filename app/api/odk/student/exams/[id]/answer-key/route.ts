...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const exam = await prisma.odkExam.findFirst({ where: { id, status: "RELEASED", resultsReleasedAt: { lte: new Date() }, attempts: { some: { studentUserId: auth.session.userId, score: { isNot: null } } } }, select: { currentVersion: { select: { files: { where: { type: "ANSWER_KEY_PDF" }, take: 1, select: { blobPathname: true, fileName: true, mimeType: true } } } } } });
