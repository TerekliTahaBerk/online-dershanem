...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const exam = await prisma.odkExam.findFirst({ where: { id, status: "DRAFT", currentVersion: { status: "DRAFT" } }, select: { family: true, examFamilyRef: { select: { code: true } }, currentVersion: { select: { id: true, sections: { select: { questions: { select: { id: true } } } } } } } });
  ...
  if (requestedOutcomeIds.length) {
    const validOutcomes = await prisma.learningOutcome.findMany({ where: { id: { in: requestedOutcomeIds }, isActive: true, unit: { subject: { version: { examFamilyRef: { code: familyCode }, status: { in: ["DRAFT", "ACTIVE"] } } } } }, select: { id: true } });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const question of parsed.data.questions) {
      await tx.odkExamQuestion.update({ where: { id: question.id }, data: { correctOption: question.correctOption, difficulty: question.difficulty, bookletPage: question.bookletPage, contentType: question.contentType, contentText: question.contentText, assetUrl: question.assetUrl } });
