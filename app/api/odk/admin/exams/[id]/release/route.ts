...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const exam = await prisma.odkExam.findUnique({ where: { id }, select: { family: true, status: true, attempts: { where: { status: { in: ["SUBMITTED", "AUTO_SUBMITTED"] } }, select: { id: true, score: { select: { attemptId: true } } } } } });
