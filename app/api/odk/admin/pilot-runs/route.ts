...
const schema = z.object({ name: z.string().trim().min(3).max(80), userIds: z.array(z.string().min(1).max(80)).min(4).max(100), requestKey: z.string().uuid() }).strict();
...
export async function POST(request: Request) {
  ...
  if (!Object.values(coverage).every((count) => count > 0)) return NextResponse.json({ error: "Pilot için admin, öğretmen, öğrenci ve en az bir veli seçilmelidir." }, { status: 400 });
  try {
