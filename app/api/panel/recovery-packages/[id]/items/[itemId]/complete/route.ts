...
export async function POST(request: Request, context: { params: Promise<{ id: string; itemId: string }> }) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!target.completedAt) await tx.recoveryPackageItem.update({ where: { id: itemId }, data: { completedAt: new Date() } });
    const remaining = await tx.recoveryPackageItem.count({ where: { packageId: id, completedAt: null } });
    const packageRow = await tx.recoveryPackage.findUniqueOrThrow({ where: { id }, include: { lesson: { select: { endsAt: true } }, _count: { select: { items: true } } } });
    const shouldComplete = remaining === 0 && packageRow.checkpointResponse !== null;
    if (shouldComplete) await tx.recoveryPackage.update({ where: { id }, data: { status: "COMPLETED", completedAt: new Date(), version: { increment: 1 } } });
    return { packageRow, completed: shouldComplete, replayed: Boolean(target.completedAt), started: completedBefore === 0 && !target.completedAt };
  });
  ...
  if (result.started) {
    await recordPanelProductEvent({
      name: "missed_lesson_recovery_started",
      properties: {
        product: "OD",
        actionKind: "COMPLETE_RECOVERY",
        reasonCode: "MISSED_LESSON",
        ageBand: recoveryAgeBand(result.packageRow.lesson.endsAt),
        evidenceBand: "NA",
        role: "STUDENT",
      },
    }, auth.session.role);
