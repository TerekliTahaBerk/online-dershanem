...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.recoveryPackage.findFirst({
    where: { id, lesson: { teacherId: auth.session.userId, status: "COMPLETED", group: { isActive: true } } },
  ...
  if (item.status !== "DRAFT") {
    if (item.publishedAt && item.version === parsed.data.expectedVersion + 1) return NextResponse.json({ published: true, planRebalanced: false, replayed: true });
    return NextResponse.json({ error: "Paket başka bir sekmede değişti." }, { status: 409 });
  }
  ...
  return NextResponse.json({ published: true, planRebalanced, replayed: false });
}
