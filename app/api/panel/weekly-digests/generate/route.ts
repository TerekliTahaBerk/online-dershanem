...
import { queueCalmDigestBatch, refreshQueuedCalmDigestBatch } from "@/lib/calm-weekly-digest-server";
...
export async function POST(request: Request) {
  ...
  const job = await queueCalmDigestBatch(auth.session.userId);
  const result = await refreshQueuedCalmDigestBatch(job.id);
  const trendBand = result?.results.length ? "STEADY" : "LIMITED_DATA";
  await recordPanelProductEvent({ name: "weekly_digest_generated", properties: { ruleVersion: "calm-digest-v1", trendBand, reused: false } }, auth.session.role);
  return NextResponse.json({ id: job.id, processedCount: result?.processedCount ?? 0, failedCount: result?.failedCount ?? 0 });
}
