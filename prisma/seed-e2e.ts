...
async function main() {
  ...
  // Tohumdaki plan bir Online Koçum planıdır (registry ürün kimliği zorunlu).
  const okProductId = (await prisma.product.findUniqueOrThrow({ where: { code: "OK" }, select: { id: true } })).id;
  ...
  await prisma.weeklyPlan.create({ data: { id: ids.foreignWeeklyPlan, studentId: ids.planForeignStudentProfile, productRefId: okProductId, weekStart: planningWeekStart(), status: "DRAFT", capacityMinutes: 45, createdById: ids.otherTeacher, tasks: { create: { scheduledFor: planningWeekStart(), position: 1, title: "Yabancı öğrenci plan görevi", durationMinutes: 20, sourceType: "REVIEW", sourceReferenceId: ids.reviewItems[2], reasonCode: "REVIEW_DUE" } } } });
