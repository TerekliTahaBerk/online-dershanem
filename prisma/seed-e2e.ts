...
async function main() {
  ...
  await prisma.interventionCase.create({ data: { id: ids.foreignInterventionCase, studentId: ids.planForeignStudentProfile, reasonCode: "PLAN_STALLED", fingerprint: "e2e-intervention-foreign-fingerprint", explanation: "1 açıklanabilir sinyal aynı öğrenci destek bölümünde birleştirildi.", suggestedAction: "Yabancı öğrencinin küçük eylemi.", evidenceCount: 3, windowStart: interventionWindowStart(), windowEnd: new Date(), dueAt: new Date(Date.now() + 86400000), ownerId: ids.otherTeacher, signals: { create: { reasonCode: "PLAN_STALLED", explanation: "Yabancı öğrencinin kontrollü açıklaması.", suggestedAction: "Yabancı öğrencinin küçük eylemi.", evidenceCount: 3 } }, activities: { create: { type: "GENERATED" } } } });
